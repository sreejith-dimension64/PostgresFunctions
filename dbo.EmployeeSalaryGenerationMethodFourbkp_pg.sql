CREATE OR REPLACE FUNCTION "dbo"."EmployeeSalaryGenerationMethodFourbkp"(
    p_HRME_ID BIGINT,
    p_MI_ID BIGINT,
    p_HRMLY_ID BIGINT,
...
BEGIN
    ...
    ELSE
        v_MONTH_START_DATE := CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-', v_DAYFROM);
        v_DAYTO := EXTRACT(DAY FROM (DATE(CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-01')) + INTERVAL '1 month - 1 day')::DATE);
    ...
    WHERE A."MI_Id" = p_MI_ID AND A."HRME_Id" = p_HRME_ID AND C."HRELTD_LWPFlag" = TRUE
          ...
          AND (("HRELT_FromDate" BETWEEN v_MONTH_START_DATE::DATE AND v_MONTH_END_DATE::DATE) 
               OR ("HRELT_ToDate" BETWEEN v_MONTH_START_DATE::DATE AND v_MONTH_END_DATE::DATE));
    ...
    ELSE
        IF v_HRC_SalApprovalFlg = TRUE THEN
            ...
            VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, v_EMPHRMDES_Id, v_HRMBD_BankName, v_HRMEB_AccountNo, NULL, FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL, NULL)
            RETURNING "HRES_ID" INTO v_NEW_IDENTITY;
        ELSE
            INSERT INTO "HR_Employee_Salary" 
            VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, v_EMPHRMDES_Id, v_HRMBD_BankName, v_HRMEB_AccountNo, NULL, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL, NULL)
            RETURNING "HRES_ID" INTO v_NEW_IDENTITY;
        END IF;
    END IF;
    ...
    LOOP
        ...
        IF v_TEMPSALCOUNT = 0 THEN
            FOR rec IN
                SELECT "A"."HRMED_Id", "A"."HRMEDP_HRMED_Id", "B"."hrmed_amount", "C"."HREED_Percentage", "D"."HRMED_EDTypeFlag", "D"."HRMED_EarnDedFlag", "D"."HRMED_AmountPercentFlag", "D"."HRMED_RoundOffFlag"
                ...
                WHERE "A"."HRMED_Id" = v_HRMEDP_HRMED_IdCUR2 AND "D"."HRMED_ActiveFlag" = TRUE AND "C"."HREED_ActiveFlag" = TRUE AND "A"."MI_Id" = p_MI_ID AND "C"."HRME_Id" = p_HRME_ID
                      AND "A"."HRMEDP_HRMED_Id" IN (SELECT "HRMED_Id" FROM "HR_Employee_EarningsDeductions" WHERE "HRME_Id" = p_HRME_ID AND "MI_Id" = p_MI_Id AND "HREED_ActiveFlag" = TRUE)
            LOOP
                v_HRMED_