CREATE OR REPLACE FUNCTION "dbo"."Exam_BBHS_Student_SubjectWise__StThird"(
    "@MI_Id" TEXT,
    "@ASMAY_Id" TEXT,
    "@ASMCL_Id" TEXT,
    "@ASMS_Id" TEXT,
...
BEGIN
    ...
    "v_SQLQUERY" := 'CREATE TEMP TABLE "NDS_Temp_StudentDetails_Amstids" AS SELECT DISTINCT "AMST_Id" FROM "ADM_M_STUDENT" WHERE "AMST_Id" IN(' || "@AMST_Id" || ') AND "MI_Id" = ' || "@MI_Id";
    ...
    SELECT * FROM (
        SELECT 
            ...
            CAST((SUM(E."ESTMPPSGE_ExamConvertedMarks") * 100.0 / SUM(D."EMPSGE_ForMaxMarkrs")) AS NUMERIC(18,2)) AS "TotalPercentage",
            Fn."ESTMP_TotalGrade" AS "EMGD_Name",
            I."ESTMPP_SectionRank"::INTEGER AS "ESTMPP_SectionRank"
        ...
        INNER JOIN "Exm"."Exm_Stu_MP_Promo_Subjectwise_Groupwise" F ON F."EMPSG_Id" = C."EMPSG_Id" AND F."ESTMPPSG_Id" = E."ESTMPPSG_Id" AND F."EMPSG_Id" = C."EMPSG_Id"
        INNER JOIN "Exm"."Exm_Stu_MP_Promo_Subjectwise" G ON G."ESTMPPS_Id" = F."ESTMPPS_Id" AND B."ISMS_Id" = G."ISMS_Id"
        INNER JOIN "Exm"."Exm_Master_Exam" H ON H."EME_Id" = D."EME_Id" AND H."EME_Id" = E."EME_Id"
        INNER JOIN "Exm"."Exm_Student_MP_Promotion" I ON I."ASMAY_Id" = G."ASMAY_Id" AND I."ASMCL_Id" = G."ASMCL_Id" AND I."ASMS_Id" = G."ASMS_Id" AND I."AMST_Id" = G."AMST_Id"
        INNER JOIN "EXM"."Exm_Student_Marks_Process" Fn ON Fn."ASMAY_Id" = "@ASMAY_Id"::BIGINT AND Fn."ASMS_Id" = "@ASMS_Id"::BIGINT 
            AND Fn."ASMCL_Id" = "@ASMCL_Id"::BIGINT AND G."AMST_Id" = Fn."AMST_Id" AND Fn."EME_Id" = H."EME_Id"
        ...
        GROUP BY E."EME_Id", C."EMPSG_GroupName", C."EMPSG_DisplayName", G."AMST_Id", A."EMGR_Id", Fn."ESTMP_TotalGrade", I."ESTMPP_SectionRank"
        ...
        SELECT 
            ...
            CAST((SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / SUM(B."ESTMPPSG_GroupMaxMarks")) AS NUMERIC(18,2)) AS "TotalPercentage",
            (SELECT L."EMGD_Name"
             FROM "Exm"."Exm_Master_Grade_Details" AS L
             WHERE ((SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / SUM(B."ESTMPPSG_GroupMaxMarks")) BETWEEN L."EMGD_From" AND L."EMGD_To" AND L."EMGR_Id" = H."EMGR_Id")
                OR ((SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / SUM(B."ESTMPPSG_GroupMaxMarks")) BETWEEN L."EMGD_To" AND L."EMGD_From" AND L."EMGR_Id" = H."EMGR_Id")
        ...
        INNER JOIN "Exm"."Exm_M_Promotion_Subjects" G ON G."EMPS_Id" = F."EMPS_Id" AND G."EMPS_AppToResultFlg" = 1 AND G."EMPS_ActiveFlag" = 1
        INNER JOIN "Exm"."Exm_M_Promotion" H ON H."EMP_Id" = G."EMP_Id" AND H."EMP_ActiveFlag" = 1 AND H."EYC_Id" = "v_EYC_Id"
        ...
        WHERE C."ASMAY_Id" = "@ASMAY_Id"::BIGINT 
            ...
            AND I."ASMCL_Id" = "@ASMCL_Id"::BIGINT 
            AND I."ASMS_Id" = "@ASMS_Id"::BIGINT
        GROUP BY F."EMPSG_GroupName", F."EMPSG_DisplayName", C."AMST_Id", H."EMGR_Id", I."ESTMPP_SectionRank"
        ...
        INNER JOIN "Exm"."Exm_Student_MP_Promotion" I ON I."ASMAY_Id" = A."ASMAY_Id" AND I."ASMCL_Id" = A."ASMCL_Id" AND I."ASMS_Id" = A."ASMS_Id" AND I."AMST_Id" = A."AMST_Id"
