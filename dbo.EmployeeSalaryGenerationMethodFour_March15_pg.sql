CREATE OR REPLACE FUNCTION "dbo"."EmployeeSalaryGenerationMethodFour_March15"(
    p_HRME_ID BIGINT,
    p_MI_ID BIGINT,
    p_HRMLY_ID BIGINT,
...
DECLARE
    ...
    cur1 CURSOR FOR SELECT "A"."HRMED_Id", "A"."HREED_Amount", "B"."HRMED_EDTypeFlag", "B"."HRMED_EarnDedFlag", "B"."HRMED_AmountPercentFlag", "B"."HRMED_RoundOffFlag", "A"."HREED_ApplicableMaxValue"
    ...
    cur2 CURSOR FOR SELECT "A"."HRMED_Id"
    ...
    cur3 REFCURSOR;
    cur4 REFCURSOR;
    cur5 CURSOR FOR SELECT "A"."HRMED_Id", "A"."HREED_Percentage", "B"."HRMED_EDTypeFlag", "B"."HRMED_EarnDedFlag", "B"."HRMED_AmountPercentFlag", "A"."HREED_Amount", "B"."HRMED_RoundOffFlag", "A"."HREED_ApplicableMaxValue"
        FROM "HR_Employee_EarningsDeductions" "A"
        LEFT JOIN "HR_Master_EarningsDeductions" "B" ON "B"."HRMED_Id" = "A"."HRMED_Id"
        WHERE "B"."HRMED_EarnDedFlag" = 'Deduction' AND "B"."HRMED_AmountPercentFlag" = 'Percentage' AND "B"."HRMED_ActiveFlag" = TRUE AND "A"."HREED_ActiveFlag" = TRUE
            AND "A"."MI_Id" = p_MI_ID AND "A"."HRME_Id" = p_HRME_ID;
    cur6 REFCURSOR;
    cur7 CURSOR FOR SELECT "hrmed_id", SUM("hrmed_amount"), "hrmed_ed_type", "hrmed_ed_flag", "hrmed_RoundOffFlag", "hrmed_amtperc_flag"
        FROM "temp_salary_comp_detail"
        GROUP BY "hrmed_id", "hrmed_ed_type", "hrmed_ed_flag", "hrmed_RoundOffFlag", "hrmed_amtperc_flag";
    curpt REFCURSOR;
    curpf REFCURSOR;
    curpfmax REFCURSOR;
BEGIN
    ...
    ELSE
        v_MONTH_START_DATE := CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-', v_DAYFROM);
        v_DAYTO := EXTRACT(DAY FROM (DATE(CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-01')) + INTERVAL '1 month - 1 day'));
    ...
    SELECT "HRMED"."HRMED_Id", "HREED"."HREED_Amount", "HRMED"."HRMED_EarnDedFlag", "HRMED"."HRMED_EDTypeFlag",
           "HRMED"."HRMED_RoundOffFlag", COALESCE("HREED_ApplicableMaxValue", 0)
    INTO v_HRMED_IdBasic, v_BASICPAY, v_HRMED_EarnDedFlagBasic, v_HRMED_EDTypeFlagBasic, v_HRMED_RoundOffFlag, v_HREED_ApplicableMaxValue
    ...
    IF v_HREED_ApplicableMaxValue <> 0 THEN
        SELECT v_HREED_ApplicableMaxValue
        INTO v_BASICPAY
    ...
    IF v_HRES_ID > 0 THEN
        IF v_HRC_SalApprovalFlg = TRUE THEN
            ...
            SET "MI_Id" = p_MI_ID, "HRME_Id" = p_HRME_ID, "HRES_Year" = v_LEAVE_YEAR, "HRES_Month" = p_IVRM_Month_Name,
                ...
                "HRES_AccountNo" = v_HRMEB_AccountNo, "HRES_ApproveFlg" = 0
            WHERE "MI_Id" = p_MI_ID AND "HRME_Id" = p_HRME_ID AND "HRES_Year" = v_LEAVE_YEAR AND "HRES_Month" = p_IVRM_Month_Name;
        ELSE
            UPDATE "HR_Employee_Salary"
            SET "MI_Id" = p_MI_ID, "HRME_Id" = p_HRME_ID, "HRES_Year" = v_LEAVE_YEAR, "HRES_Month" = p_IVRM_Month_Name,
                ...
                "HRES_ToDate" = v_MONTH_END_DATE::DATE, "HRMGT_Id" = v_EMPHRMGT_Id, "HRMD_Id" = v_EMPHRMD_Id,
                "HRMDES_Id" = v_EMPHRMDES_Id, "HRES_BankCashFlag" = v_HRME_PaymentType, "HRES_BankCode" = v_HRMBD_BankName,
                "HRES_AccountNo" = v_HRMEB_AccountNo, "HRES_ApproveFlg" = 1
            WHERE "MI_Id" = p_MI_ID AND "HRME_Id" = p_HRME_ID AND "HRES_Year" = v_LEAVE_YEAR AND "HRES_Month" = p_IVRM_Month_Name;
        END IF;
    ...
    ELSE
        IF v_HRC_SalApprovalFlg = TRUE THEN
            ...
            VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, v_EMPHRMDES_Id, v_HRMBD_BankName, v_HRMEB_AccountNo, NULL, 0)
            RETURNING "HRES_Id" INTO v_NEW_IDENTITY;
        ELSE
            INSERT INTO "HR_Employee_Salary"("MI_Id", "HRME_Id", "HRES_Year", "HRES_Month", "HRES_WorkingDays", "HRES_DailyRates", "HRES_EPF", "HRES_FPF", "HRES_Ac21", "HRES_Ac22", "HRES_Ac5", "HRES_FromDate", "HRES_ToDate", "HRES_ArrearRegFlag", "HRES_BankCashFlag", "HRMGT_Id", "HRMD_Id", "HRMDES_Id", "HRES_BankCode", "HRES_AccountNo", "HRES_ESIEmplr", "HRES_ApproveFlg")
            VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, v_EMPHRMDES_Id, v