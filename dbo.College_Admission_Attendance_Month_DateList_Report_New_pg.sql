CREATE OR REPLACE FUNCTION "dbo"."College_Admission_Attendance_Month_DateList_Report_New"(
    "p_asmay_id" VARCHAR,
    "p_mi_id" VARCHAR,
    "p_month" VARCHAR,
    "p_amco_id" VARCHAR,
...
DECLARE
    ...
    "v_sqlquery" TEXT;
    "v_cursorValue" VARCHAR;
    "v_Cl" VARCHAR;
    "v_C2" VARCHAR;
    "v_query1" TEXT;
    "v_cols" VARCHAR;
    "v_monthyearsd" VARCHAR;
    "v_monthyearsd1" VARCHAR;
    "v_cols1" VARCHAR;
    "v_startDate" DATE;
    "v_endDate" DATE;
BEGIN

    DROP TABLE IF EXISTS "dbo"."StudentPeriodWiseAttendance_Temp";
    ...
    'stuadm."AMCST_AdmNo" AS "admno", stuadm."AMCST_RegistrationNo" AS "regno", ' ||
    '"AMCO_CourseName" AS "coursename", "AMB_BranchName" AS "branchname", ' ||
    '"AMSE_SEMName" AS "semestername", "ACMS_SectionName" AS "sectionname", ' ||
    '"ISMS_SubjectName" AS "subjectname", ' ||
    'EXTRACT(DAY FROM att."ACSA_AttendanceDate"::TIMESTAMP)::INT AS "ACSA_AttendanceDate", ' ||
    '"AMCO_Order", "AMB_Order", "AMSE_SEMOrder", ' ||
    '"TTMP_PeriodName" || '':'' || (CASE WHEN "ACSAS_ClassAttended" = TRUE THEN ''P'' ' ||
    'WHEN "ACSAS_ClassAttended" = FALSE THEN ''A'' END) AS "P_ATT" ' ||
    ...
    'AND att."ACSA_ActiveFlag" = TRUE ' ||
    'AND att."ISMS_Id" = ' || "p_isms_id" || ' AND stuadm."AMCST_SOL" = ''S'' ' ||
    'AND EXTRACT(MONTH FROM att."ACSA_AttendanceDate") = ' || "p_month" || ' ' ||
    'AND EXTRACT(YEAR FROM att."ACSA_AttendanceDate") = ' || "v_Year"::VARCHAR || ' ' ||
    'AND stuadm."AMCST_ActiveFlag" = TRUE ' ||
    'ORDER BY "student", "AMCO_Order", "AMB_Order", "AMSE_SEMOrder" ' ||
    'LIMIT 100; ' ||
    'SELECT DISTINCT A."AMCST_Id", A."student", A."admno", A."regno", A."coursename", ' ||
    'A."branchname", A."semestername", A."sectionname", A."subjectname", A."ACSA_AttendanceDate", ' ||
    'A."AMCO_Order", A."AMB_Order", A."AMSE_SEMOrder", ' ||
    '(SELECT STRING_AGG(B."P_ATT", '','') ' ||
    'FROM "StudentPeriodWiseAttendance_Temp" B ' ||
    'WHERE A."AMCST_Id" = B."AMCST_Id" AND B."admno" = A."admno" ' ||
    'AND A."ACSA_AttendanceDate" = B."ACSA_AttendanceDate") AS "P_ATT" ' ||
