CREATE OR REPLACE FUNCTION "dbo"."CLG_Auto_Fee_Group_Mapping_Transport_LocationRequest"(
    p_MI_Id bigint,
    p_ASMAY_Id bigint,
    p_AMCST_Id bigint,
    p_UserId bigint
...
DECLARE
    ...
    v_FMC_TransportFeeLocationFlag boolean;
...
BEGIN
    ...
    BEGIN
        ...
        AND "FMG_Id" IN (
            ...
            WHERE "TRLFM_WayFlag" = v_selectflag 
            AND "MI_Id" = p_MI_Id
        ) 
        ...
        ELSE
            DELETE FROM "TempGroupLocationRequest_College";
        END IF;

        FOR yearly_fee_rec IN (
            SELECT "FMG_Id" FROM "TempGroupLocationRequest_College"
            UNION
            SELECT "FMG_Id" FROM "TRN"."TR_Location_FeeGroup_Mapping" 
            WHERE "MI_Id" = p_MI_Id AND "TRLFM_WayFlag" = v_selectflag
        )
        LOOP
            ...
            IF v_Rcount = 0 THEN
                INSERT INTO "CLG"."Fee_College_Master_Student_GroupHead"(
                    ...
                    p_MI_Id, p_AMCST_Id, p_ASMAY_Id, v_FMG_Id, 1
                );
            END IF;
            ...
            AND a."ASMAY_Id" = v_previous_yearid::bigint;
            ...
            IF v_AMSE_Id_Prev != 0 THEN
                ...
                FROM "CLG"."Adm_Master_Semester" 
                WHERE "MI_Id" = p_MI_Id 
                AND "AMSE_SEMOrder" = v_currentorderid::int;
            ...
            SELECT DISTINCT COALESCE("FMG_BatchwiseFeeApplFlg", false) 
            INTO v_FMG_BatchwiseFeeApplFlg 
            FROM "Fee_Master_Group" 
            WHERE "MI_Id" = p_MI_Id 
            AND "FMG_Id" = v_FMG_Id;

            SELECT COALESCE("FMC_TransportFeeLocationFlag", false) 
            INTO v_FMC_TransportFeeLocationFlag 
            FROM "Fee_Master_Configuration" 
            WHERE "MI_Id" = p_MI_Id;

            DROP TABLE IF EXISTS "Fee_CollegeSemesterWiseAmount_TransLocationRQ_Temp";

            IF (v_FMG_BatchwiseFeeApplFlg = true) AND v_FMC_TransportFeeLocationFlag = true THEN
                CREATE TEMP TABLE "Fee_CollegeSemesterWiseAmount_TransLocationRQ_Temp" AS
            ...
            ELSIF (v_FMG_BatchwiseFeeApplFlg = false) AND v_FMC_TransportFeeLocationFlag = true THEN
                CREATE TEMP TABLE "Fee_CollegeSemesterWiseAmount_TransLocationRQ_Temp" AS
                ...
                INNER JOIN "CLG"."Fee_College_Master_Amount_Semesterwise" FS 
                    ...
                    AND FA."AMB_Id" = v_AMB_Id 
                    AND FS."AMSE_Id" = v_AMSE_Id
                INNER JOIN "Fee_Master_Head" FMH 
                    ON FMH."FMH_Id" = FA."FMH_Id" 
                    AND FMH."FMH_Flag" = 'T'
                INNER JOIN "Fee_Master_Group" FMG 
                    ON FMG."FMG_Id" = FA."FMG_Id" 
            ...
            FOR instidssem_rec IN (
                ...
                FROM "Fee_CollegeSemesterWiseAmount_TransLocationRQ_Temp"
            )
            LOOP
                ...
                IF v_selectflag = 'One Way' AND v_FMC_TransportFeeLocationFlag = true THEN
                    SELECT DISTINCT "TRMLAMT_OneWayAmount" INTO v_FCMAS_Amount 
                    FROM "TRN"."TR_Location_Amount" TLA
                    INNER JOIN "TRN"."TR_Location_FeeGroup_Mapping" TLFM 
                        ON TLFM."TRML_Id" = TLA."TRML_Id"
                ...
                ELSIF v_selectflag = 'Two Way' AND v_FMC_TransportFeeLocationFlag = true THEN
