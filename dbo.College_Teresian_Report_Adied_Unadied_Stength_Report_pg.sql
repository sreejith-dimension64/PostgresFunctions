CREATE OR REPLACE FUNCTION "dbo"."College_Teresian_Report_Adied_Unadied_Stength_Report"(
    "p_Mi_Id" TEXT,
    "p_ASMAY_Id" TEXT,
    "p_evenorodd" TEXT
)
...
BEGIN
    ...
    FOR "v_course", "v_courseid", "v_courseorder" IN
        SELECT "AMCO_CourseName", "mc"."AMCO_Id", "AMCO_Order"
        ...
        WHERE "yc"."ASMAY_Id" = "p_ASMAY_Id" 
            AND "yc"."MI_Id" = "p_Mi_Id" 
            AND "mc"."MI_Id" = "p_Mi_Id" 
            AND "ACAYC_ActiveFlag" = 1 
            AND "AMCO_ActiveFlag" = 1
        ORDER BY "AMCO_Order"
    LOOP

        FOR "v_branch", "v_branchid", "v_branchorder" IN
            SELECT "AMB_BranchName", "mb"."AMB_Id", "AMB_Order"
            FROM "clg"."Adm_College_AY_Course" "yc"
            INNER JOIN "clg"."Adm_Master_Course" "mc" ON "yc"."AMCO_Id" = "mc"."amco_id"
        ...
        LOOP
            ...
            FOR "v_semester", "v_semesterid", "v_semesterorder" IN
                SELECT DISTINCT "AMSE_SEMName", "mse"."AMSE_Id", "AMSE_SEMOrder"
                FROM "clg"."Adm_College_AY_Course" "yc"
                INNER JOIN "clg"."Adm_Master_Course" "mc" ON "yc"."AMCO_Id" = "mc"."amco_id"
                INNER JOIN "clg"."Adm_College_AY_Course_Branch" "ycb" ON "ycb"."ACAYC_Id" = "yc"."ACAYC_Id"
                INNER JOIN "clg"."Adm_Master_Branch" "mb" ON "ycb"."AMB_Id" = "mb"."AMB_Id"
                ...
                WHERE "yc"."ASMAY_Id" = "p_ASMAY_Id" 
                    ...
                    AND "mc"."MI_Id" = "p_Mi_Id" 
                    AND "yc"."AMCO_Id" = "v_courseid" 
                    AND "ycb"."AMB_Id" = "v_branchid"
                    AND "ACAYC_ActiveFlag" = 1 
                    AND "AMCO_ActiveFlag" = 1 
                    AND "AMB_ActiveFlag" = 1 
                    AND "AMSE_ActiveFlg" = 1 
                    AND "AMSE_EvenOdd" = "p_evenorodd" 
                    AND "ycb"."ACAYCB_ActiveFlag" = 1
            ...
            LOOP
                ...
                WHERE "ys"."ASMAY_Id" = "p_ASMAY_Id" 
                    AND "ms"."MI_Id" = "p_Mi_Id" 
                    AND "mse"."AMSE_EvenOdd" = "p_evenorodd"
                    AND "ys"."AMSE_Id" = "v_semesterid" 
                    AND "ys"."AMCO_Id" = "v_courseid" 
                    AND "ys"."AMB_Id" = "v_branchid" 
                    AND "ms"."AMCST_SOL" = 'S' 
                    AND "ms"."AMCST_ActiveFlag" = 1 
                    AND "ys"."ACYST_ActiveFlag" = 1
                GROUP BY "AMCO_CourseName", "AMB_BranchName", "AMSE_SEMName", "AMCO_Order", "AMB_Order", "mse"."AMSE_SEMOrder"
                ORDER BY "AMCO_Order", "AMB_Order", "mse"."AMSE_SEMOrder";
