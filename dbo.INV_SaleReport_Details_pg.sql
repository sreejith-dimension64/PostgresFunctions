CREATE OR REPLACE FUNCTION "INV"."INV_SaleReport_Details"(
    "p_MI_Id" BIGINT,
    "p_type" VARCHAR(20)
)
RETURNS TABLE (
...
BEGIN
    ...
    ELSIF "p_type" = 'Staff' THEN
        ...
        WHERE "a"."HRME_Id" = "d"."HRME_Id" 
          AND "c"."INVMSL_Id" = "d"."INVMSL_Id" 
          AND "a"."MI_Id" = "p_MI_Id"
        ORDER BY "a"."HRME_EmployeeOrder"::BIGINT;

    ELSIF "p_type" = 'Customer' THEN
        RETURN QUERY
        SELECT DISTINCT 
            "a"."INVMC_Id",
            "a"."INVMC_CustomerName"::TEXT,
            NULL::TEXT,
            NULL::TEXT
