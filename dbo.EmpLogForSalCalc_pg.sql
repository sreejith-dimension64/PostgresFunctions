...
CREATE OR REPLACE FUNCTION "dbo"."EmpLogForSalCalc"(
    "p_date" VARCHAR(10),
    "p_month" VARCHAR(2),
    "p_year" VARCHAR(4),
...
BEGIN
    ...
    IF UPPER("p_punchtype") = 'LATE' THEN
        ...
        i."HRMGT_EmployeeGroupType", b."FOEP_PunchDate", c."FOEST_IHalfLoginTime", j."FOEPD_PunchTime", f."MI_Id",
        b."FOEP_Id", c."FOEST_DelayPerShiftHrMin", f."HRME_EmployeeFirstName", f."HRME_EmployeeMiddleName", f."HRME_EmployeeLastName"';
    ...
    LOOP
        ...
        "v_punchdate" := "rec"."punchdate";
        "v_lateby" := "rec"."lateby";

        "v_lateby_New" := COALESCE(CAST(CAST("v_lateby" AS INT) / 60 AS VARCHAR) || ':' || LPAD(CAST(CAST("v_lateby" AS INT) % 60 AS VARCHAR), 2, '0'), '0');
    ...
    FOR "rec" IN 
        ...
        WHERE "MI_Id" = "p_miid"
    LOOP
        "v_RHRME_Id" := "rec"."HRME_Id";
        "v_Rpunchdate" := "rec"."punchdate";

        SELECT MAX("FOEP_PunchDate"), "FOEP_RunningLateInsNos"
        INTO "v_punchdate", "v_RFOEP_RunningLateInsNos"
        FROM "FO"."FO_Emp_Punch" EP
        WHERE EP."MI_Id" = "p_miid" AND EP."FOEP_HolidayPunchFlg" = 0 
        AND ("FOEP_RunningLateInsNos" >= 1) AND "HRME_Id" = "v_RHRME_Id"
        ...
        IF ("v_RFOEP_RunningLateInsNos" >= 1) THEN
            ...
            WHERE "MI_Id" = "p_miid" AND "HRME_Id" = "v_RHRME_Id" 
            AND CAST("FOEP_PunchDate" AS DATE) = CAST("v_Rpunchdate" AS DATE);
            ...
            FROM "FO"."FO_Emp_Punch"
            WHERE "MI_Id" = "p_miid" AND "HRME_Id" = "v_RHRME_Id" 
            AND "FOEP_RunningLateInsNos" <> 0 AND "FOEP_Id" = "v_FOEP_Id_U";
            ...
            IF ("v_Exceedno" = 0) THEN
                ...
                WHERE "MI_Id" = "p_miid" AND "HRMLY_Id" = "v_HRMLY_Id" 
                AND "HRME_Id" = "v_RHRME_Id" AND "HRELT_Status" = 'Approved'
                AND CAST("v_Rpunchdate" AS DATE) BETWEEN CAST("HRELT_FromDate" AS DATE) AND CAST("HRELT_ToDate" AS DATE);
                ...
                ELSE
                    ...
                    WHERE "HRML_Id" = "v_leave" AND "HRELS_CBLeaves" > 0 
                    AND "MI_Id" = "p_miid" AND "HRME_Id" = "v_RHRME_Id";
                    ...
                    IF ("v_rcount" > 0) THEN
                        ...
                        SET "HRELS_TransLeaves" = "v_HRLPC_LateLOP", 
                            "HRELS_CBLeaves" = "HRELS_CBLeaves" - "HRELS_TransLeaves"
                        WHERE "MI_Id" = "p_miid" AND "HRME_Id" = "v_RHRME_Id" 
                        AND "HRML_Id" = "v_leave" AND "HRMLY_Id" = "v_HRMLY_Id";
                        ...
                        INSERT INTO "HR_Emp_Leave_Trans"(
                            ...
                            "p_miid", "v_RHRME_Id", "v_HRMLY_Id", "v_leave", "v_Rpunchdate", "v_Rpunchdate",
                            "v_HRLPC_LateLOP", CURRENT_TIMESTAMP, "v_HRML_LeaveCode", 'Approved',
                            CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP
                        );

                        SELECT MAX("HRELT_Id") INTO "v_HRELT_Id"
                        FROM "HR_Emp_Leave_Trans" 
                        WHERE "MI_Id" = "p_miid" AND CAST("HRELT_FromDate" AS DATE) = CAST("v_Rpunchdate" AS DATE) 
                        AND "HRME_Id" = "v_RHRME_Id";
                        ...
                        INSERT INTO "HR_Emp_Leave_Trans_Details"(
                            ...
                            "v_HRLPC_LateLOP", 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                        );
                    ...
                    ELSE
                        ...
                        INSERT INTO "HR_Emp_Leave_Trans"(
                            ...
                            "p_miid", "v_RHRME_Id", "v_HRMLY_Id", "v_leave", "v_Rpunchdate", "v_Rpunchdate",
                            "v_HRLPC_LateLOP", CURRENT_TIMESTAMP, 'LWP', 'Approved',
                            CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP
                        );
        ...
        FROM "FO"."FO_Emp_Punch" EP
        WHERE EP."MI_Id" = "p_miid" AND EP."FOEP_HolidayPunchFlg" = 0 
        AND ("FOEP_RunningLateInsNos" IS NULL) AND "HRME_Id" = "v_RHRME_Id"
        GROUP BY EP."MI_Id", "HRME_Id", "FOEP_RunningLateInsNos"
        LIMIT 1;
