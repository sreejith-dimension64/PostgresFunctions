CREATE OR REPLACE FUNCTION "dbo"."INV_SALESReport_OD"(
    p_MI_Id BIGINT,
...
BEGIN
    ...
    IF (p_typeflag = 'Overall') THEN
        ...
        ELSIF p_optionflag = 'Student' THEN
            v_Slqdymaic := '
            SELECT "AMST_Id", "AMST_AdmNo", "membername", SUM("saleQty") AS "saleQty", SUM("salePrice") AS "salePrice",
            ...
            FROM (
                ...
                (COALESCE("ADM"."AMST_FirstName", '''') || 
                 CASE WHEN COALESCE("AMST_MiddleName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "AMST_MiddleName" END || 
                 CASE WHEN COALESCE("AMST_LastName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "AMST_LastName" END) AS "membername",
                "MI"."INVMI_Id",
                CAST("MST"."INVTSL_SalesQty" / NULLIF("Mastersalescount", 0) AS DECIMAL(18,2)) AS "saleQty",
                ("MST"."INVTSL_SalesPrice") AS "salePrice",
                ("INVTSL_DiscountAmt") AS "saleDiscount",
                ...
                WHERE "MSL"."INVMSL_ActiveFlg" = TRUE AND "MSST"."AMST_Id" = "ADM"."AMST_Id"
        ...
        ELSIF p_optionflag = 'Staff' THEN
            ...
            (COALESCE("HRE"."HRME_EmployeeFirstName", '''') || 
             CASE WHEN COALESCE("HRME_EmployeeMiddleName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "HRME_EmployeeMiddleName" END || 
             CASE WHEN COALESCE("HRME_EmployeeLastName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "HRME_EmployeeLastName" END) AS "membername",
            SUM("MST"."INVTSL_SalesQty") AS "saleQty",
            SUM("MST"."INVTSL_SalesPrice") AS "salePrice",
            SUM("INVTSL_DiscountAmt") AS "saleDiscount",
            SUM("INVTSL_TaxAmt") AS "saleTax",
            SUM("INVTSL_Amount") AS "saleAmount",
            SUM("MSL"."INVMSL_SalesValue") AS "saleValue"
            FROM "INV"."INV_M_Sales" "MSL"
            INNER JOIN "INV"."INV_T_Sales" "MST" ON "MST"."INVMSL_Id" = "MSL"."INVMSL_Id"
            INNER JOIN "INV"."INV_Master_Item" "MI" ON "MI"."INVMI_Id" = "MST"."INVMI_Id"
    ...
    IF (p_typeflag = 'Detailed') THEN
        ...
        ELSIF p_optionflag = 'Saleno' THEN
            v_Slqdymaic := '
            SELECT DISTINCT "MSL"."INVMSL_Id", "MST"."INVMI_Id", "MI"."INVMI_ItemName", "ADM"."AMST_Id", "ADM"."AMST_AdmNo",
            (COALESCE("ADM"."AMST_FirstName", '''') || 
             CASE WHEN COALESCE("AMST_MiddleName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "AMST_MiddleName" END || 
             CASE WHEN COALESCE("AMST_LastName", '''') IN ('''', ''0'') THEN '''' ELSE '' '' || "AMST_LastName" END) AS "membername",
            ...
            ("INVTSL_DiscountAmt") AS "INVTSL_DiscountAmt",
