CREATE OR REPLACE FUNCTION "dbo"."IVRM_ModulesPagesMapping_New"(
    p_SMI_Id bigint,
    p_MI_Id bigint,
    p_UserName text
)
...
BEGIN
    ...
    INSERT INTO "Applicationuser"("AccessFailedCount","ConcurrencyStamp","Email","EmailConfirmed","LockoutEnabled","LockoutEnd","NormalizedEmail","NormalizedUserName","PasswordHash","PhoneNumber","PhoneNumberConfirmed","SecurityStamp","TwoFactorEnabled","UserName","Entry_Date","Machine_Ip_Address","UserImagePath","RoleTypeFlag","CreatedDate","UpdatedDate","Name","CreatedBy","UpdatedBy")
    SELECT "AccessFailedCount","ConcurrencyStamp","Email","EmailConfirmed","LockoutEnabled","LockoutEnd","NormalizedEmail",p_UserName AS "NormalizedUserName",'AQAAAAEAACcQAAAAEGARxyK/NchjUhbaTLWFLaRoajzOq97Z9auxTu298V4cvp/lulgUFuJ0MftIeeSdyA==' AS "PasswordHash","PhoneNumber","PhoneNumberConfirmed","SecurityStamp","TwoFactorEnabled",p_UserName AS "UserName","Entry_Date","Machine_Ip_Address","UserImagePath","RoleTypeFlag",CURRENT_TIMESTAMP AS "CreatedDate",CURRENT_TIMESTAMP AS "UpdatedDate","Name","CreatedBy","UpdatedBy"
    FROM "Applicationuser"
    LIMIT 1;

    SELECT "id" INTO v_AppId FROM "Applicationuser" WHERE "NormalizedUserName" = p_UserName ORDER BY "id" DESC LIMIT 1;

    SELECT "IVRMRT_Id" INTO v_RoleTypeId FROM "IVRM_Role_Type" WHERE "IVRMRT_Role" = 'ADMIN' ORDER BY "IVRMRT_Id" DESC LIMIT 1;
    SELECT "roleid" INTO v_roleid FROM "ApplicationUserRole" WHERE "RoleTypeId" = v_RoleTypeId ORDER BY "roleid" DESC LIMIT 1;

    v_URCount := 0;
    SELECT COUNT(*) INTO v_URCount FROM "ApplicationUserRole" WHERE "USERID" = v_AppId;
    
    IF v_URCount = 0 THEN
        INSERT INTO "ApplicationUserRole"("UserId","RoleId","RoleTypeId","CreatedDate","UpdatedDate")
        SELECT v_AppId, v_roleid, v_RoleTypeId, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP;
    END IF;
    ...
    v_ULIRcount := 0;
    SELECT COUNT(*) INTO v_ULIRcount FROM "IVRM_User_Login_Institutionwise" WHERE "MI_Id" = p_MI_Id AND "Id" = v_AppId;
    ...
    IF v_ULIRcount = 0 THEN
        ...
        SELECT p_MI_Id, v_AppId, "CreatedDate", "UpdatedDate", "Activeflag", "IVRMULI_PaymentAlertFlg", "IVRMULI_SubExpAlertFlg" 
    ...
    SELECT COUNT(*) INTO v_ISMPCount FROM "IVRM_SMS_MAIL_PARAMETER";
    
    IF v_ISMPCount = 0 THEN
        INSERT INTO "IVRM_SMS_MAIL_PARAMETER" ("ISMP_ID", "ISMP_NAME", "CreatedDate", "UpdatedDate") VALUES 
    ...
    SELECT COUNT(*) INTO v_IRMPCount FROM "IVRM_Role_MobileApp_Privileges" WHERE "MI_Id" = p_MI_Id;
