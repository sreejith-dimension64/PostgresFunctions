CREATE OR REPLACE FUNCTION "dbo"."Exam_BBHS_Student_SubjectWise_Details"(
    "p_MI_Id" TEXT,
    "p_ASMAY_Id" TEXT,
    "p_ASMCL_Id" TEXT,
    "p_ASMS_Id" TEXT,
...
DECLARE
    "v_EYC_Id" BIGINT;
    "v_EMCA_Id" BIGINT;
    "ref_cursor1" REFCURSOR := 'result_cursor';
BEGIN
    ...
    RAISE NOTICE 'v_EYC_Id: %', "v_EYC_Id";
    RAISE NOTICE 'v_EMCA_Id: %', "v_EMCA_Id";

    /* STUDENT DETAILS */
    IF "p_FLAG" = '1' THEN
        OPEN "ref_cursor1" FOR
        SELECT 
            A."AMST_Id",
    ...
    ELSIF "p_FLAG" = '4' THEN
        ...
        SELECT * FROM (
            SELECT 
                ...
                CAST((SUM(E."ESTMPPSGE_ExamConvertedMarks") * 100.0 / NULLIF(SUM(D."EMPSGE_ForMaxMarkrs"), 0)) AS DECIMAL(18,2)) AS "TotalPercentage",
            ...
            SELECT 
                ...
                CAST((SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / NULLIF(SUM(B."ESTMPPSG_GroupMaxMarks"), 0)) AS DECIMAL(18,2)) AS "TotalPercentage",
                (SELECT L."EMGD_Name"
                 FROM "Exm"."Exm_Master_Grade_Details" AS L
                 WHERE ((SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / NULLIF(SUM(B."ESTMPPSG_GroupMaxMarks"), 0) BETWEEN L."EMGD_From" AND L."EMGD_To" 
                        AND L."EMGR_Id" = H."EMGR_Id")
                     OR (SUM(B."ESTMPPSG_GroupObtMarks") * 100.0 / NULLIF(SUM(B."ESTMPPSG_GroupMaxMarks"), 0) BETWEEN L."EMGD_To" AND L."EMGD_From" 
                        AND L."EMGR_Id" = H."EMGR_Id"))
