CREATE OR REPLACE FUNCTION "dbo"."Fee_ClassWise_Settlement_Report" (
    p_MI_Id BIGINT,
    p_ASMAY_Id BIGINT,
    p_Fromdate VARCHAR,
    p_Todate VARCHAR,
...
BEGIN
    ...
    WHERE "Fee_Y_Payment"."mi_id" = ' || p_MI_Id || ' 
      ...
      AND "Fee_Y_Payment"."user_Id" = ' || p_userid || ' ' || v_where_condition;
    ...
    v_SQLQuery2 := 'SELECT "AMST_Id", "StudentName", "ClassName", "SectionName", "AdmNo", "UTR_No", "Transactionid", "PaymentId", ' || 
                   ...
                   ' FROM crosstab(
                       ''SELECT "AMST_Id", "StudentName", "ClassName", "SectionName", "AdmNo", "UTR_No", "Transactionid", "PaymentId", "FeeName", SUM("paidAmount") 
                         FROM "FeeSettementReport" 
                         GROUP BY "AMST_Id", "StudentName", "ClassName", "SectionName", "AdmNo", "UTR_No", "Transactionid", "PaymentId", "FeeName" 
                         ORDER BY 1'',
                       ''SELECT DISTINCT "FeeName" FROM "FeeHead_Temp" ORDER BY 1''
                   ) AS ct("AMST_Id" BIGINT, "StudentName" TEXT, "ClassName" TEXT, "SectionName" TEXT, "AdmNo" TEXT, "UTR_No" TEXT, "Transactionid" TEXT, "PaymentId" BIGINT, ' || 
                   v_PivotColumnNames || ' NUMERIC)';
