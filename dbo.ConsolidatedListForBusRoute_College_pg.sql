CREATE OR REPLACE FUNCTION "dbo"."ConsolidatedListForBusRoute_College"(
    p_MI_Id bigint,
    p_ASMAY_Id bigint
)
RETURNS TABLE(
...
BEGIN
    ...
    DROP TABLE IF EXISTS "StudentBusRouteClg_Temp";

    CREATE TEMP TABLE "StudentBusRouteClg_Temp"(
        routeid bigint,
        "RouteName" varchar(300),
        twonewcount bigint,
        tworegcount bigint,
    ...
    WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASMAY_Id AND "TRRSCO_ActiveFlg"=1 
    AND "AMCST_Id" IN (
        ...
        WHERE AA."MI_Id"=p_MI_Id AND BB."ASMAY_Id"=p_ASMAY_Id AND AA."AMCST_SOL"='S' 
        AND AA."AMCST_ActiveFlag"=1 AND BB."ACYST_ActiveFlag"=1
    );

    SELECT count(DISTINCT "AMCST_Id") INTO v_totaltransport1 
    FROM "Adm_Student_Trans_Appl_College" 
    WHERE "MI_Id"=p_MI_Id AND "ASTACO_ForAY"=p_ASMAY_Id AND "ASTACO_ActiveFlag"=1 
    AND "ASTACO_Amount">0 AND "ASTACO_ApplStatus"='Approved'
    AND "AMCST_Id" IN (
        SELECT DISTINCT AA."AMCST_Id" 
        FROM "CLG"."Adm_Master_College_Student" AS AA 
        INNER JOIN "CLG"."Adm_College_Yearly_Student" AS BB ON AA."AMCST_Id"=BB."AMCST_Id" 
    ...
    LOOP
        ...
        FROM "TRN"."TR_Student_Route_College" 
        WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=v_prevacademic AND "TRRSCO_ActiveFlg"=1 
        AND "AMCST_Id" IN (
            SELECT DISTINCT "AMCST_Id" 
            FROM "TRN"."TR_Student_Route_College" 
            WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASMAY_Id AND "TRRSCO_ActiveFlg"=1 
        ...
        AND ("ASTACO_PickUp_TRMR_Id"=v_routeid OR "ASTACO_Drop_TRMR_Id"=v_routeid) 
        AND ("ASTACO_PickUp_TRMR_Id"<>0 AND "ASTACO_Drop_TRMR_Id"<>0) 
        AND "ASTACO_Amount">0 AND "ASTACO_ApplStatus"='Approved' AND "ASTACO_Regnew"='Regular' 
        AND "AMCST_Id" NOT IN (
            SELECT DISTINCT "AMCST_Id" 
            FROM "TRN"."TR_Student_Route_College" 
            WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=v_prevacademic AND "TRRSCO_ActiveFlg"=1 
        ...
        FROM "Adm_Student_Trans_Appl_College" 
        WHERE "MI_Id"=p_MI_Id AND "ASTACO_ForAY"=p_ASMAY_Id AND "ASTACO_ActiveFlag"=1 
        AND "ASTACO_ApplStatus"='Approved' AND "ASTACO_PickUp_TRMR_Id"=v_routeid 
        ...
        AND ("ASTACO_Drop_TRMR_Id"=0 OR "ASTACO_Drop_TRMR_Id" IS NULL) 
        AND "ASTACO_ApplStatus"='Approved' AND "ASTACO_Regnew"='Regular' 
        AND "AMCST_Id" NOT IN(
            SELECT DISTINCT "AMCST_Id" 
            FROM "TRN"."TR_Student_Route_College" 
            WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=v_prevacademic AND "TRRSCO_ActiveFlg"=1 
        ...
        FROM "Adm_Student_Trans_Appl_College" 
        WHERE "MI_Id"=p_MI_Id AND "ASTACO_ForAY"=p_ASMAY_Id AND "ASTACO_ActiveFlag"=1 
        AND "ASTACO_ApplStatus"='Approved' AND "ASTACO_Drop_TRMR_Id"=v_routeid 
        ...
        INSERT INTO "StudentBusRouteClg_Temp" 
    ...
    RETURN QUERY SELECT * FROM "StudentBusRouteClg_Temp";
