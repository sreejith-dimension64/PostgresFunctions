CREATE OR REPLACE FUNCTION "dbo"."Fee_DetailedAccountPosition_AsonDate"(
    "mi_id" BIGINT,
    "asmay_Id" BIGINT,
    "asmcl_id" BIGINT,
    "amsc_id" BIGINT,
...
DECLARE
    ...
    "Debit" BIGINT;
    "LastYear" BIGINT;
    "IndRoute_cursor" REFCURSOR;
    "routecursor_rec" RECORD;
    "DynamicD1" TEXT;
    "DynamicD2" TEXT;
BEGIN
    ...
    IF "Type" = 'headwise' THEN
        ...
        "sqlquery" := 'WITH cte AS (
            SELECT DISTINCT "Adm_School_M_Class"."ASMCL_ClassName" AS "ClassName", "Fee_Master_Head"."FMH_FeeName" AS "FeeName", "fee_t_installment"."FTI_Name",
            SUM("fee_student_status"."FSS_NetAmount") AS "NetAmt", SUM("FSS_ConcessionAmount") AS "ConcessAmt", SUM("FSS_RebateAmount") AS "RebateAmt",
            SUM("FSS_WaivedAmount") AS "WaivedAmt", SUM("FSS_FineAmount") AS "FineAmt", SUM("FSS_PaidAmount") AS "CollectionAmt",
            SUM("FSS_OBArrearAmount") AS "OBArrearAmt", SUM("FSS_ToBePaid") AS "tobepaid"
            ...
            AND ("fee_student_status"."MI_Id" = ' || "mi_id"::TEXT || ') AND "Fee_Student_Status"."FMG_Id" IN 
            (SELECT DISTINCT "FMG_Id" FROM "Fee_Master_Group_Grouping_Groups" WHERE "Fee_Master_Group_Grouping_Groups"."fmgg_id" IN 
            (SELECT DISTINCT "fmgg_id" FROM "Fee_Master_Group_Grouping" WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "fmgg_id" IN (' || "fmgg_id" || ')))
        ...
        SUM("WaivedAmt") AS "Waive Off", SUM("FineAmt") AS "Fine", (SUM("CollectionAmt") - SUM("FineAmt")) AS "Collection",
    ...
    ELSIF "Type" = 'route' THEN
        ...
        LOOP
            "trmr_id" := "routecursor_rec"."TRMR_Id";
            "RouteName" := "routecursor_rec"."TRMR_RouteName";
            
            "sqlquery" := '
            SELECT SUM("Charges") AS "Charges", SUM("Concession") AS "Concession", SUM("Rebate/Schlorship") AS "Rebate/Schlorship",
            SUM("Waive Off") AS "Waive Off", SUM("Fine") AS "Fine", SUM("Collection") AS "Collection", 
            SUM("Debit Balance") AS "Debit Balance", SUM("Last Year Due") AS "Last Year Due"
            FROM (
                SELECT SUM("FSS_NetAmount") AS "Charges", SUM("FSS_ConcessionAmount") AS "Concession", 
                SUM("FSS_RebateAmount") AS "Rebate/Schlorship", SUM("FSS_WaivedAmount") AS "Waive Off",
                SUM("FSS_FineAmount") AS "Fine", SUM("FSS_PaidAmount") AS "Collection", SUM("FSS_ToBePaid") AS "Debit Balance",
                SUM("FSS_OBArrearAmount") AS "Last Year Due", "Fee_Master_Group"."FMG_GroupName"
                FROM "Fee_Master_Group"
                ...
                WHERE ("Adm_School_Y_Student"."ASMAY_Id" = ' || "asmay_Id"::TEXT || ') 
                AND ("Fee_Student_Status"."MI_Id" = ' || "mi_id"::TEXT || ') 
                ...
                AND ("Fee_Student_Status"."AMST_Id" IN (
                    ...
                    WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Id" IN (
                        SELECT "TRMR_Id" FROM "TRN"."TR_Master_Route" 
                        WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Id" = ' || "trmr_id"::TEXT || '
                    ) AND "ASMAY_Id" = ' || "asmay_Id"::TEXT || ' AND "TRSR_ActiveFlg" = true
                    UNION
                    SELECT DISTINCT "AMST_Id" FROM "TRN"."TR_Student_Route" 
                    WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Drop_Route" IN (
                        SELECT "TRMR_Id" FROM "TRN"."TR_Master_Route" 
                        WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Id" = ' || "trmr_id"::TEXT || '
                    ) AND "ASMAY_Id" = ' || "asmay_Id"::TEXT || ' AND "TRSR_ActiveFlg" = true
                    AND "AMST_Id" NOT IN (
                        ...
                        WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Id" IN (
                            SELECT "TRMR_Id" FROM "TRN"."TR_Master_Route" 
                            WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "TRMR_Id" <> 0
                        ) AND "ASMAY_Id" = ' || "asmay_Id"::TEXT || ' AND "TRSR_ActiveFlg" = true
                    )
                ))
            ...
            FOR "IndRoute_cursor" IN EXECUTE "sqlquery"
            LOOP
                FETCH "IndRoute_cursor" INTO "Charges", "Concession", "Rebate", "Waive", "Fine", "Collection", "Debit", "LastYear";
                EXIT WHEN NOT FOUND;
        ...
        EXECUTE 'SELECT "RouteName", SUM("Charges") AS "Charges", SUM("Concession") AS "Concession", 
    ...
    ELSIF "Type" = 'All' THEN
        ...
        "sqlquery" := 'WITH cte AS (
            ...
            SUM("fee_student_status"."FSS_NetAmount") AS "NetAmt", SUM("FSS_ConcessionAmount") AS "ConcessAmt",
            SUM("FSS_RebateAmount") AS "RebateAmt", SUM("FSS_WaivedAmount") AS "WaivedAmt", SUM("FSS_FineAmount") AS "FineAmt",
            SUM("FSS_PaidAmount") AS "CollectionAmt", SUM("FSS_OBArrearAmount") AS "OBArrearAmt", SUM("FSS_ToBePaid") AS "tobepaid"
            FROM "Fee_Master_Group"
            INNER JOIN "Fee_Student_Status" ON "Fee_Master_Group"."FMG_Id" = "Fee_Student_Status"."FMG_Id" AND "Fee_Master_Group"."MI_Id" = ' || "mi_id"::TEXT || '
            INNER JOIN "Fee_Master_Head" ON "Fee_Student_Status"."FMH_Id" = "Fee_Master_Head"."FMH_Id" AND "Fee_Master_Head"."MI_Id" = ' || "mi_id"::TEXT || '
            INNER JOIN "Adm_M_Student" ON "Adm_M_Student"."AMST_Id" = "Fee_Student_Status"."AMST_Id" AND "Adm_M_Student"."MI_Id" = ' || "mi_id"::TEXT || '
    ...
    ELSIF "Type" = 'individual' AND "status" = 'true' THEN
        ...
        "sqlquery" := 'WITH cte AS (
            ...
            "FMH_FeeName" AS "FeeName", "FTI_Name" AS "TName", "FSS_NetAmount" AS "NetAmt", "FSS_ConcessionAmount" AS "ConcessAmt",
            "FSS_RebateAmount" AS "RebateAmt", "FSS_WaivedAmount" AS "WaivedAmt", "FSS_FineAmount" AS "FineAmt",
            "FSS_PaidAmount" AS "CollectionAmt", "FSS_OBArrearAmount" AS "OBArrearAmt", "FSS_ToBePaid" AS "tobepaid",
            ...
            INNER JOIN "Fee_T_Due_Date" ON "Fee_T_Due_Date"."FMA_Id" = "Fee_Student_Status"."FMA_Id"
            WHERE ("Adm_School_Y_Student"."ASMAY_Id" = ' || "asmay_Id"::TEXT || ') AND ("Fee_Student_Status"."FMG_Id" IN (' || "fmg_id" || '))
            AND ("fee_student_status"."MI_Id" = ' || "mi_id"::TEXT || ') AND ("Adm_School_M_Class"."ASMCL_Id" = ' || "asmcl_id"::TEXT || ')
            AND ("adm_school_m_section"."asms_id" = ' || "amsc_id"::TEXT || ')
            AND "Fee_Student_Status"."FMG_Id" IN (SELECT DISTINCT "FMG_Id" FROM "Fee_Master_Group_Grouping_Groups" 
            WHERE "Fee_Master_Group_Grouping_Groups"."fmgg_id" IN (SELECT DISTINCT "fmgg_id" FROM "Fee_Master_Group_Grouping" 
            WHERE "mi_id" = ' || "mi_id"::TEXT || ' AND "fmgg_id" IN (' || "fmgg_id" || ')))
            AND ("fee_student_status"."ASMAY_Id" = ' || "asmay_Id"::TEXT || ')
        )
        SELECT "admno", "StudentName", SUM("NetAmt") AS "Charges", SUM("ConcessAmt") AS "Concession",
        SUM("RebateAmt") AS "Rebate/Schlorship", SUM("WaivedAmt") AS "Waive Off", SUM("FineAmt") AS "Fine",
        (SUM("CollectionAmt") - SUM("FineAmt")) AS "Collection", SUM("tobepaid") AS "Debit Balance",
        SUM("OBArrearAmt") AS "Last Year Due"
        FROM cte GROUP BY "admno", "StudentName"';
    ...
    ELSIF "Type" = 'individual' AND "status" = 'false' THEN
        ...
        "DynamicD1" := 'WITH cte AS (
            SELECT DISTINCT "Adm_M_Student"."AMST_Id", "Adm_M_Student"."AMST_Admno" AS "admno",
            (COALESCE("AMST_FirstName", '''') || '' '' || COALESCE("AMST_MiddleName", '''') || '' '' || COALESCE("AMST_LastName", '''')) AS "StudentName",
            "FMH_FeeName" AS "FeeName", "FTI_Name" AS "TName", "FSS_NetAmount" AS "NetAmt", "FSS_ConcessionAmount" AS "ConcessAmt",
            "FSS_RebateAmount" AS "RebateAmt", "FSS_WaivedAmount" AS "WaivedAmt", "FSS_FineAmount" AS "FineAmt",
            "FSS_PaidAmount" AS "CollectionAmt", "FSS_OBArrearAmount" AS "OBArrearAmt", "FSS_ToBePaid" AS "tobepaid",
            ...
            INNER JOIN "Fee_Master_Head" ON "Fee_Student_Status"."FMH_Id" = "Fee_Master_Head"."FMH_Id" AND "Fee_Master_Head"."MI_Id" = ' || "mi_id"::TEXT || '
            INNER JOIN "Adm_M_Student" ON "Adm_M_Student"."AMST_Id" = "Fee_Student_Status"."AMST_Id" AND "Adm_M_Student"."MI_Id" = ' || "mi_id"::TEXT || '
            INNER JOIN "Adm_School_Y_Student" ON "Adm_School_Y_Student"."AMST_Id" = "