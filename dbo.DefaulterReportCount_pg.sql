CREATE OR REPLACE FUNCTION "dbo"."DefaulterReportCount"(
    p_MI_Id TEXT,
    p_ASMAY_Id TEXT,
    p_FMT_Id TEXT
)
...
BEGIN
    ...
    ASMCL_ClassName AS (
        ...
        WHERE "FYP"."MI_Id" = ' || p_MI_Id || ' AND "FYP"."ASMAY_Id" = ' || p_ASMAY_Id || ' 
        AND DATE("FYP_Date") = CURRENT_DATE 
        AND "ASYS"."ASMAY_Id" = ' || p_ASMAY_Id || ' 
        AND ("ASYS"."AMAY_ActiveFlag" = 1) 
        AND ("AMS"."AMST_SOL" = ''S'') 
        AND ("AMS"."AMST_ActiveFlag" = 1) 
        AND "FMT"."FMT_Id" = ' || p_FMT_Id || '
    ...
    TotalStrength AS (
        ...
        AND "Fee_Master_Terms"."FMT_Id" = ' || p_FMT_Id || '
    ...
    Defaulters AS (
        ...
        AND ("Fee_Master_Terms"."FMT_Id" IN (
            ...
            WHERE "FMTFH_Id" IN (
                ...
                WHERE "MI_Id" = ' || p_MI_Id || ' 
                AND DATE("FMTFHDD_DueDate") < DATE("FYP"."FYP_Date") 
    ...
    SELECT 
        ...
        "TPC"."TodayPaidCount", 
        "TC"."TodayCollection", 
        "TNPC"."TillNowPaidCount", 
        "TNTC"."TillNowTotalCollection", 
        "TS"."TotalStrength", 
        "DS"."Defaulters", 
        "dd"."Defaulterscnt"
    FROM MasterTerms "MT"
    ...
    LEFT JOIN ASMCL_ClassName "asmcl" ON "MT"."FMT_Id" = "asmcl"."FMT_Id"
