CREATE OR REPLACE FUNCTION "dbo"."IndExamMarksCalculation_Sthomas" (
    p_MI_Id bigint,
    p_ASMAY_Id bigint,
    p_ASMCL_Id bigint,
    p_ASMS_Id bigint,
...
DECLARE
    ...
    v_ODcountU bigint;
    rec_exmsubject RECORD;
    rec_examsubwise RECORD;
    rec_studentmarks RECORD;
    rec_studentsub RECORD;
    rec_subsubmed RECORD;
...
BEGIN
    ...
    WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASMAY_Id AND "ASMCL_Id"=p_ASMCL_Id AND "ASMS_Id"=p_ASMS_Id AND "EME_Id"=p_EME_Id;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='Exm' AND table_name='Exm_Student_Marks_Pro_Sub_SubSubject')
    THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='Exm' AND table_name='Exm_Student_Marks_Process_Subjectwise')
    THEN
        DELETE FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASMAY_Id AND "ASMCL_Id"=p_ASMCL_Id AND "ASMS_Id"=p_ASMS_Id AND "EME_Id"=p_EME_Id;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='Exm' AND table_name='Exm_Student_Marks_Process')
    THEN
    ...
    SELECT DISTINCT "ESM"."AMST_Id", "ESM"."MI_Id", "ESM"."ASMAY_Id", "ESM"."ASMCL_Id", "ESM"."ASMS_Id", "EYCES"."EYCES_Id",
    "ESM"."ISMS_Id", "ESM"."EME_Id", "EYCES_AplResultFlg", COALESCE("EYCES_MarksEntryMax",0) AS "EYCES_MarksEntryMax", 
    COALESCE("EYCES_MaxMarks",0) AS "EYCES_MaxMarks", COALESCE("EYCES_MinMarks",0) AS "EYCES_MinMarks", 
    "EYCES"."EMGR_Id", COALESCE("ESM"."ESTM_Marks",0) AS "ESTM_Marks", "ESM"."ESTM_MarksGradeFlg", 
    "ESTM_Grade", "ESTM_Flg", COALESCE("n"."EME_Id",0) AS "SPTEME_Id"
    FROM "Adm_M_Student" AS f
    INNER JOIN "Adm_School_Y_Student" AS h ON h."AMST_Id" = f."AMST_Id" AND h."ASMAY_Id" = p_ASMAY_Id AND h."ASMCL_Id" = p_ASMCL_Id AND h."ASMS_Id" = p_ASMS_Id AND f."mi_id"=p_MI_Id
    INNER JOIN "Exm"."Exm_Category_Class" AS "ECC" ON "ECC"."MI_Id"=p_MI_Id AND "ECC"."ASMAY_Id"=p_ASMAY_Id AND "ECC"."ASMCL_Id"=h."ASMCL_Id" AND "ECC"."ASMS_Id"=h."ASMS_Id"
    INNER JOIN "Exm"."Exm_Yearly_Category" AS "EYC" ON "EYC"."MI_Id"=p_MI_Id AND "EYC"."ASMAY_Id"=p_ASMAY_Id AND "ECC"."EMCA_Id"="EYC"."EMCA_Id"
    INNER JOIN "Exm"."Exm_Yearly_Category_Exams" AS "EYCE" ON "EYCE"."EYC_Id"="EYC"."EYC_Id" AND "EYCE"."EME_Id"=p_EME_Id
    INNER JOIN "Exm"."Exm_Yrly_Cat_Exams_Subwise" "EYCES" ON "EYCES"."EYCE_Id"="EYCE"."EYCE_Id"
    INNER JOIN "Exm"."Exm_Student_Marks" AS "ESM" ON "ESM"."AMST_Id"=f."AMST_Id" AND "ESM"."ISMS_Id"="EYCES"."ISMS_Id" 
        AND "ESM"."MI_Id"="EYC"."MI_Id" AND "ESM"."EME_Id"="EYCE"."EME_Id" AND "ESM"."ASMAY_Id"="ECC"."ASMAY_Id" 
        AND "ESM"."ASMS_Id"="ECC"."ASMS_Id" AND "ESM"."ASMCL_Id"="ECC"."ASMCL_Id"
        AND "ESM"."MI_Id" = p_MI_Id AND "ESM"."ASMAY_Id" = p_ASMAY_Id AND "ESM"."ASMCL_Id" = p_ASMCL_Id 
        AND "ESM"."ASMS_Id" = p_ASMS_Id AND "ESM"."EME_Id"=p_EME_Id
    INNER JOIN "Exm"."Exm_Studentwise_Subjects" AS n ON n."ISMS_Id" = "EYCES"."ISMS_Id" AND n."AMST_Id"=f."AMST_Id" 
        AND n."MI_Id" = p_MI_Id AND n."ASMAY_Id" = p_ASMAY_Id AND n."ASMCL_Id" = p_ASMCL_Id AND n."ASMS_Id" = p_ASMS_Id 
        AND n."ESTSU_ActiveFlg"=true AND (n."EME_Id"=p_EME_Id OR n."EME_Id" IS NULL OR n."EME_Id"=0)
    WHERE "EYC"."MI_Id"=p_MI_Id AND "EYC"."ASMAY_Id"=p_ASMAY_Id AND "ECC"."ASMCL_Id"=p_ASMCL_Id 
        AND "ECC"."ASMS_Id"=p_ASMS_Id AND "EYCE"."EME_Id"=p_EME_Id
        AND "ECC"."ECAC_ActiveFlag"=true AND "EYC_ActiveFlg"=true AND "EYCE_ActiveFlg"=true 
        AND "EYCES_ActiveFlg"=true AND "ESTM_ActiveFlg"=true
    ORDER BY "ESM"."AMST_Id";

    FOR rec_exmsubject IN SELECT * FROM "Exmsubjectdetails"
    LOOP
        v_AMST_Id := rec_exmsubject."AMST_Id";
        v_MI_Id := rec_exmsubject."MI_Id";
        v_ASMAY_Id := rec_exmsubject."ASMAY_Id";
        v_ASMCL_Id := rec_exmsubject."ASMCL_Id";
        v_ASMS_Id := rec_exmsubject."ASMS_Id";
        v_EYCES_Id := rec_exmsubject."EYCES_Id";
        v_ISMS_Id := rec_exmsubject."ISMS_Id";
        v_EME_Id := rec_exmsubject."EME_Id";
        v_EYCES_AplResultFlg := rec_exmsubject."EYCES_AplResultFlg";
        v_EYCES_MarksEntryMax := rec_exmsubject."EYCES_MarksEntryMax";
        v_EYCES_MaxMarks := rec_exmsubject."EYCES_MaxMarks";
        v_EYCES_MinMarks := rec_exmsubject."EYCES_MinMarks";
        v_EMGR_Id := rec_exmsubject."EMGR_Id";
        v_ESTM_Marks := rec_exmsubject."ESTM_Marks";
        v_ESTM_MarksGradeFlg := rec_exmsubject."ESTM_MarksGradeFlg";
        v_ESTM_Grade := rec_exmsubject."ESTM_Grade";
        v_ESTM_Flg := rec_exmsubject."ESTM_Flg";
        v_SPTEME_Id := rec_exmsubject."SPTEME_Id";

        IF (v_SPTEME_Id > 0) THEN
            SELECT "PT"."EMPATY_Id", "PT"."EMGR_Id" INTO v_EMPATY_Id, v_PTEMGR_Id
            FROM "Exm"."Exm_Yrly_Cat_Exams_Subwise_PT" "PT"
            INNER JOIN "Exm"."Exm_Student_Examwise_PT" "SEPT" ON "SEPT"."EMPATY_Id"="PT"."EMPATY_Id"
            WHERE "SEPT"."MI_Id"=p_MI_Id AND "SEPT"."ASMAY_Id"=p_ASMAY_Id AND "SEPT"."ASMCL_Id"=p_ASMCL_Id 
                AND "SEPT"."ASMS_Id"=p_ASMS_Id AND "SEPT"."AMST_Id"=v_AMST_Id AND "SEPT"."EME_Id"=p_EME_Id 
                AND "PT"."EYCES_Id"=v_EYCES_Id AND "SEPT"."ISMS_Id"=v_ISMS_Id AND "PT"."EYCESPT_ActiveFlg"=true 
                AND "SEPT"."ESEWPT_ActiveFlg"=true;

            v_EMGR_Id := v_PTEMGR_Id;
        END IF;
        ...
        IF (v_ESTM_Flg='AB') THEN
            ...
            WHERE "ESM"."MI_Id"=p_MI_Id AND "ESM"."ASMAY_Id"=p_ASMAY_Id AND "ESM"."ASMCL_Id"=p_ASMCL_Id 
                AND "ESM"."ASMS_Id"=p_ASMS_Id AND "ESM"."AMST_Id"=v_AMST_Id AND "ESM"."EME_Id"=p_EME_Id 
        ...
        IF v_ESTM_MarksGradeFlg='M' THEN
            ...
            WHERE ((v_GradeMarksPercentage BETWEEN "EMGD_From" AND "EMGD_To")) AND "EMGR_Id"=v_EMGR_Id
            LIMIT 1;

        ELSIF v_ESTM_MarksGradeFlg='G' THEN
            v_ESTMPS_ObtainedGrade := v_ESTM_Grade;
            
            SELECT "EMGD_Id" INTO v_EMGD_Id
            FROM "Exm"."Exm_Master_Grade_Details" 
            WHERE "EMGD_Name"=v_ESTM_Grade AND "EMGR_Id"=v_EMGR_Id;
        END IF;
        ...
        INSERT INTO "Exm"."Exm_Student_Marks_Process_Subjectwise" (
            ...
            "ESTMPS_Medical_MaxMarks", "ESTMPS_AplResultFlg", "EMGD_Id", "ESTMPS_Percentage", "EMPATY_Id"
        )
        VALUES(
            p_MI_Id, p_ASMAY_Id, p_ASMCL_Id, p_ASMS_Id, v_AMST_Id, v_ISMS_Id, p_EME_Id, v_EYCES_MaxMarks, 
            v_ESTM_Marks, v_ESTMPS_ObtainedGrade, v_ESTMPS_PassFailFlg, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 
            v_EYCES_MaxMarks, v_EYCES_AplResultFlg, v_EMGD_Id, v_ESTMPS_Percentage, v_EMPATY_Id
        );

        SELECT "EYCES_SubExamFlg", "EYCES_SubSubjectFlg" INTO v_EYCES_SubExamFlg, v_EYCES_SubSubjectFlg
        FROM "Exm"."Exm_Category_Class" a
        INNER JOIN "Exm"."Exm_Yearly_Category" b ON b."EMCA_Id"=a."EMCA_Id" AND b."MI_Id"=a."MI_Id" 
            AND b."ASMAY_Id"=a."ASMAY_Id" AND b."EYC_ActiveFlg"=true AND a."ECAC_ActiveFlag"=true
        INNER JOIN "Exm"."Exm_Yearly_Category_Exams" c ON c."EYC_Id"=b."EYC_Id" AND c."EYCE_ActiveFlg"=true
        INNER JOIN "Exm"."Exm_Yrly_Cat_Exams_Subwise" d ON d."EYCE_Id"=c."EYCE_Id" AND d."EYCES_ActiveFlg"=true
        WHERE a."MI_Id"=p_MI_Id AND a."ASMAY_Id"=p_ASMAY_Id AND a."ASMCL_Id"=p_ASMCL_Id AND a."ASMS_Id"=p_ASMS_Id 
            AND c."EME_Id"=p_EME_Id AND d."ISMS_Id"=v_ISMS_Id;
    ...
    FOR rec_examsubwise IN 
        ...
        INNER JOIN "Exm"."Exm_Student_Marks" AS "ESM" ON "ESM"."ISMS_Id"="EYCES"."ISMS_Id" AND "ESM"."MI_Id"="EYC"."MI_Id" 
            AND "ESM"."EME_Id"="EYCE"."EME_Id" AND "ESM"."ASMAY_Id"="ECC"."ASMAY_Id" AND "ESM"."ASMS_Id"="ECC"."ASMS_Id" 
            AND "ESM"."ASMCL_Id"="ECC"."ASMCL_Id"
        WHERE "EYC"."MI_Id"=p_MI_Id AND "EYC"."ASMAY_Id"=p_ASMAY_Id AND "ECC"."ASMCL_Id"=p_ASMCL_Id 
            AND "ECC"."ASMS_Id"=p_ASMS_Id AND "EYCE"."EME_Id"=p_EME_Id
            AND "EYC"."EYC_ActiveFlg"=true AND "EYCE"."EYCE_ActiveFlg"=true AND "EYCES"."EYCES_ActiveFlg"=true 
            AND "ESM"."ESTM_ActiveFlg"=true
        ORDER BY "ESM"."ISMS_Id"
    LOOP
        v_MI_Id := rec_examsubwise."MI_Id";
        v_ASMAY_Id := rec_examsubwise."ASMAY_Id";
        v_ASMCL_Id := rec_examsubwise."ASMCL_Id";
        v_ASMS_Id := rec_examsubwise."ASMS_Id";
        v_ISMS_Id := rec_examsubwise."ISMS_Id";
        v_EME_Id := rec_examsubwise."EME_Id";

        SELECT SUM(COALESCE("ESTMPS_ObtainedMarks", 0)) INTO v_Section_Totalmarks
        FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASMAY_Id AND "ASMCL_Id"=p_ASMCL_Id AND "ASMS_Id"=p_ASMS_Id 
            AND "EME_Id"=p_EME_Id AND "ISMS_Id"=v_ISMS_Id
        GROUP BY "ASMCL_Id", "ASMS_Id", "ISMS_Id";

        SELECT COUNT("AMST_Id") INTO v_Section_Totalcount
        FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"=p_MI_Id AND "ASMAY_Id"=p_ASM