CREATE OR REPLACE FUNCTION "dbo"."Exm_Multiple_Exam_Cumulative_Report"(
    p_MI_Id TEXT,
    p_ASMAY_Id TEXT,
    p_ASMCL_Id TEXT,
    p_ASMS_Id TEXT,
...
DECLARE
    ...
    v_EYCES_AplResultFlg TEXT;
    v_EYCES_MaxMarks DECIMAL(18,2);
    v_SUBJECT_PERCENTAGE DECIMAL(18,2);
    v_MARKS_OBTAINED DECIMAL(18,2);
    v_MAX_MARKS DECIMAL(18,2);
    v_EMGR_GRADENAME TEXT;
    v_FINAL_PERCENTAGE DECIMAL(18,2);
    v_FINAL_MARKS_OBTAINED DECIMAL(18,2);
    v_FINAL_MAX_MARKS DECIMAL(18,2);
...
IF p_FLAG = '1' THEN
    ...
    FOR student_rec IN
        ...
        WHERE A."ASMAY_Id"::TEXT = p_ASMAY_Id 
            ...
            AND B."MI_Id"::TEXT = p_MI_Id
    LOOP
        v_AMST_Id_New := student_rec."AMST_Id"::TEXT;
        ...
        v_ASMCL_Id_New := student_rec."ASMCL_Id"::TEXT;
        v_ASMS_Id_New := student_rec."ASMS_Id"::TEXT;
        v_ASMAY_Id_New := student_rec."ASMAY_Id"::TEXT;
        ...
        FOR subject_rec IN
            ...
            INNER JOIN "EXM"."Exm_Category_Class" H ON H."ASMAY_Id" = E."ASMAY_Id" 
                ...
                AND H."ECAC_ActiveFlag" = TRUE
                AND H."ASMCL_Id"::TEXT = v_ASMCL_Id_New 
                AND H."ASMAY_Id"::TEXT = v_ASMAY_Id_New 
                AND H."ASMS_Id"::TEXT = v_ASMS_Id_New
            ...
            INNER JOIN "EXM"."Exm_Yearly_Category" J ON J."ASMAY_Id" = E."ASMAY_Id" 
                ...
                AND J."ASMAY_Id"::TEXT = v_ASMAY_Id_New
            ...
            WHERE A."AMST_Id"::TEXT = v_AMST_Id_New 
                AND C."AMST_Id"::TEXT = v_AMST_Id_New 
                AND C."ASMAY_Id"::TEXT = v_ASMAY_Id_New 
                AND A."ASMAY_Id"::TEXT = v_ASMAY_Id_New
                AND A."ASMCL_Id"::TEXT = v_ASMCL_Id_New 
                AND C."ASMCL_Id"::TEXT = v_ASMCL_Id_New 
                AND A."ASMS_Id"::TEXT = v_ASMS_Id_New 
                AND C."ASMS_Id"::TEXT = v_ASMS_Id_New 
                AND A."ESTSU_ActiveFlg" = TRUE
        ...
        LOOP
            ...
            v_EYCES_AplResultFlg := subject_rec."EYCES_AplResultFlg"::TEXT;
            ...
            v_SQLQUERY := 'CREATE TEMP TABLE "STUDENTMARKSLIST" AS ' ||
                         ...
                         'WHERE "AMST_Id"::TEXT = ' || quote_literal(v_AMST_Id_New) ||
                         ' AND "ASMAY_Id"::TEXT = ' || quote_literal(v_ASMAY_Id_New) ||
                         ' AND "ASMCL_Id"::TEXT = ' || quote_literal(v_ASMCL_Id_New) ||
                         ' AND "ASMS_Id"::TEXT = ' || quote_literal(v_ASMS_Id_New) ||
                         ' AND "ISMS_Id"::TEXT = ' || quote_literal(v_ISMS_Id) ||
                         ' AND "MI_Id"::TEXT = ' || quote_literal(p_MI_Id) ||
                         ' AND "EME_Id"::TEXT IN (' || p_EME_Id || ') ' ||
            ...
            WHERE "AMST_Id"::TEXT = v_AMST_Id_New 
                AND "ISMS_Id"::TEXT = v_ISMS_Id;

            v_MARKS_OBTAINED := COALESCE(v_MARKS_OBTAINED, 0);
            v_MAX_MARKS := COALESCE(v_MAX_MARKS, 0);

            v_SUBJECT_PERCENTAGE := CASE WHEN v_MAX_MARKS = 0 THEN 0 
                                         ELSE ROUND((v_MARKS_OBTAINED * 100 / NULLIF(v_MAX_MARKS, 0))::NUMERIC, 2) 
                                    END;
            ...
            WHERE A."EMGR_Id"::TEXT = p_EMGR_Id 
                AND B."MI_Id"::TEXT = p_MI_Id 
                AND (v_SUBJECT_PERCENTAGE BETWEEN A."EMGD_From" AND A."EMGD_To");

            v_EMGR_GRADENAME := COALESCE(v_EMGR_GRADENAME, '');

            INSERT INTO "STUDENT_MULTIPLE_EXAM_WISE_CUMULATIVE_REPORT" (
                "AMST_Id", "STUDENTNAME", "ADMNO", "REGNO", "CLASSNAME", "SECTIONNAME", "YEARNAME", "ROLLNO",
                "ISMS_Id", "SUBJECTNAME", "MARKSOBTAINED", "MAXMARKS", "PERCENTAGE", "GRADE",
                "ASMCL_Id", "ASMS_Id", "ASMAY_Id"
            ) VALUES (
                v_AMST_Id_New::BIGINT, v_STUDENTNAME, v_AMST_AdmNo, v_AMST_RegistrationNo, v_ASMCL_ClassName,
                v_ASMC_SectionName, v_ASMAY_Year, v_AMAY_RollNo, v_ISMS_Id::BIGINT, v_ISMS_SubjectName,
                v_MARKS_OBTAINED, v_MAX_MARKS, v_SUBJECT_PERCENTAGE, v_EMGR_GRADENAME,
                v_ASMCL_Id_New::BIGINT, v_ASMS_Id_New::BIGINT, v_ASMAY_Id_New::BIGINT
            );
        ...
        v_FINAL_MARKS_OBTAINED := COALESCE(v_FINAL_MARKS_OBTAINED, 0);
        v_FINAL_MAX_MARKS := COALESCE(v_FINAL_MAX_MARKS, 0);

        v_FINAL_PERCENTAGE := CASE WHEN v_FINAL_MAX_MARKS = 0 THEN 0 
                                   ELSE ROUND((v_FINAL_MARKS_OBTAINED * 100 / NULLIF(v_FINAL_MAX_MARKS, 0))::NUMERIC, 2) 
                              END;

        RAISE NOTICE '@FINAL_PERCENTAGE: %', v_FINAL_PERCENTAGE;
        ...
        v_FINAL_GRADENAME := COALESCE(v_FINAL_GRADENAME, '');
        ...
        INSERT INTO "STUDENT_MULTIPLE_EXAM_WISE_CUMULATIVE_REPORT" (
            ...
            v_AMST_Id_New::BIGINT, v_STUDENTNAME, v_AMST_AdmNo, v_AMST_RegistrationNo, v_ASMCL_ClassName,
            v_ASMC_SectionName, v_ASMAY_Year, v_AMAY_RollNo, 1000000, 'TOTALFINALMARKS',
            v_FINAL_MARKS_OBTAINED, v_FINAL_MAX_MARKS, v_FINAL_PERCENTAGE, v_FINAL_GRADENAME,
            v_ASMCL_Id_New::BIGINT, v_ASMS_Id_New::BIGINT, v_ASMAY_Id_New::BIGINT
        );
        ...
        INSERT INTO "STUDENT_MULTIPLE_EXAM_WISE_CUMULATIVE_REPORT" (
            ...
            v_AMST_Id_New::BIGINT, v_STUDENTNAME, v_AMST_AdmNo, v_AMST_RegistrationNo, v_ASMCL_ClassName,
            v_ASMC_SectionName, v_ASMAY_Year, v_AMAY_RollNo, 10000001, 'TOTALFINALGRADE',
            v_FINAL_MARKS_OBTAINED, v_FINAL_MAX_MARKS, v_FINAL_PERCENTAGE, v_FINAL_GRADENAME,
            v_ASMCL_Id_New::BIGINT, v_ASMS_Id_New::BIGINT, v_ASMAY_Id_New::BIGINT
        );
    ...
    RETURN QUERY 
    SELECT * FROM "STUDENT_MULTIPLE_EXAM_WISE_CUMULATIVE_REPORT" 
...
ELSIF p_FLAG = '2' THEN
    ...
    RETURN QUERY
    SELECT DISTINCT 
        NULL::BIGINT, NULL::TEXT, NULL::TEXT, NULL::TEXT, NULL::TEXT, NULL::TEXT, NULL::TEXT, NULL::TEXT,
        E."ISMS_Id",
        F."ISMS_SubjectName",
        NULL::DECIMAL(18,2), NULL::DECIMAL(18,2), NULL::DECIMAL(18,2), NULL::TEXT,
        NULL::BIGINT, NULL::BIGINT, NULL::BIGINT, NULL::TEXT, NULL::TEXT, NULL::TEXT,
        F."ISMS_OrderFlag" AS "EYCES_SubjectOrder",
        F."ISMS_OrderFlag" AS "ISMS_OrderFlag"
    ...
    INNER JOIN "Exm"."Exm_Studentwise_Subjects" G ON G."ISMS_Id" = F."ISMS_Id" 
        AND G."ASMAY_Id"::TEXT = p_ASMAY_Id 
        AND G."ASMCL_Id"::TEXT = p_ASMCL_Id 
        AND G."ASMS_Id"::TEXT = p_ASMS_Id
    WHERE A."MI_Id"::TEXT = p_MI_Id 
        AND A."ASMAY_Id"::TEXT = p_ASMAY_Id 
        AND A."ASMCL_Id"::TEXT = p_ASMCL_Id 
        AND A."ASMS_Id"::TEXT = p_ASMS_Id 
        AND C."ASMAY_Id"::TEXT = p_ASMAY_Id 
        AND C."EYC_ActiveFlg" = TRUE
        AND A."ECAC_ActiveFlag" = TRUE 
        AND D."EYCE_ActiveFlg" = TRUE 
        AND E."EYCES_ActiveFlg" = TRUE 
        AND E."EYCES_AplResultFlg" = TRUE
...
ELSIF p_FLAG = '3' THEN
    ...
    SELECT DISTINCT 
        ...
        NULL::BIGINT, NULL::TEXT, NULL::DECIMAL(18,2), NULL::DECIMAL(18,2), NULL::DECIMAL(18,2), NULL::TEXT,
        NULL::BIGINT, NULL::BIGINT, NULL::BIGINT,
        CONCAT(a."AMST_FirstName", ' ', a."AMST_MiddleName", ' ', a."AMST_LastName", ' ') AS "AMST_FirstName",
        a."AMST_AdmNo",
        b."AMAY_RollNo",
