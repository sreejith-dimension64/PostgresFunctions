CREATE OR REPLACE FUNCTION "dbo"."Exm_Marks_Entry_Report_Details"(
    p_MI_Id TEXT,
    p_ASMAY_Id TEXT,
    p_ASMCL_Id TEXT,
    p_ASMS_Id TEXT,
...
BEGIN
    ...
    IF p_ASMS_Id > '0' THEN
        ...
        FOR rec_subject IN 
            ...
            WHERE c."EMCA_Id" = v_EMCA_Id AND c."ASMAY_Id" = p_ASMAY_Id AND e."EME_Id" = p_EME_Id 
                AND a."EYCGS_ActiveFlg" = 1 AND b."EYCG_ActiveFlg" = 1 AND c."EYC_ActiveFlg" = 1
        ...
        LOOP
            ...
            SELECT COUNT(1) INTO v_student_rowcount
            ...
            WHERE b."ASMAY_Id" = p_ASMAY_Id AND b."ASMCL_Id" = p_ASMCL_Id AND b."ASMS_Id" = p_ASMS_Id 
                AND b."AMAY_ActiveFlag" IN (0, 1) AND a."AMST_ActiveFlag" IN (0, 1) AND a."amst_sol" IN ('S')
                AND c."ASMAY_Id" = p_ASMAY_Id AND c."ASMCL_Id" = p_ASMCL_Id 
                AND c."ASMS_Id" = p_ASMS_Id AND c."ISMS_Id" = v_ISMS_Id;
            ...
            WHERE a."mi_id" = p_MI_Id AND a."ASMAY_Id" = p_ASMAY_Id 
                AND a."ASMCL_Id" = p_ASMCL_Id AND a."ASMS_Id" = p_ASMS_Id
                AND a."EME_Id" = p_EME_Id AND a."ISMS_Id" = v_ISMS_Id 
                AND "AMST_Id" IN (
                    SELECT a."AMST_Id" 
                    FROM "adm_M_student" a 
                    INNER JOIN "adm_school_Y_student" b ON a."amst_id" = b."amst_id"
                    INNER JOIN "Exm"."Exm_Studentwise_Subjects" c ON c."AMST_Id" = b."AMST_Id" AND c."ESTSU_ActiveFlg" = 1
                    WHERE b."ASMAY_Id" = p_ASMAY_Id AND b."ASMCL_Id" = p_ASMCL_Id AND b."ASMS_Id" = p_ASMS_Id 
                        AND b."AMAY_ActiveFlag" IN (1) AND a."AMST_ActiveFlag" IN (1) AND a."amst_sol" = 'S'
                        AND c."ASMAY_Id" = p_ASMAY_Id AND c."ASMCL_Id" = p_ASMCL_Id 
                        AND c."ASMS_Id" = p_ASMS_Id AND c."ISMS_Id" = v_ISMS_Id
                );
            ...
            INSERT INTO "Marks_Entry_Report" 
            VALUES(p_MI_Id, p_ASMCL_Id, p_ASMS_Id, v_ASMCL_ClassName, v_ASMC_SectionName, 
                   v_ISMS_Id::BIGINT, v_ISMS_SubjectName, v_ISMS_order, v_employeename, 
                   v_enteryflag, v_enteryflag_color, v_SourceMarksEntry);
        END LOOP;
    ...
    ELSE
        ...
        LOOP
            ...
            LOOP
                ...
                INNER JOIN "adm_school_Y_student" b ON a."amst_id" = b."amst_id"
                INNER JOIN "Exm"."Exm_Studentwise_Subjects" c ON c."AMST_Id" = b."AMST_Id" AND c."ESTSU_ActiveFlg" = 1
                WHERE b."ASMAY_Id" = p_ASMAY_Id AND b."ASMCL_Id" = p_ASMCL_Id AND b."ASMS_Id" = v_ASMS_Id_New 
                    AND b."AMAY_ActiveFlag" IN (0, 1) AND a."AMST_ActiveFlag" IN (0, 1) AND a."amst_sol" = 'S'
                    AND c."ASMAY_Id" = p_ASMAY_Id AND c."ASMCL_Id" = p_ASMCL_Id 
                    AND c."ASMS_Id" = v_ASMS_Id_New AND c."ISMS_Id" = v_ISMS_Id;
                ...
                INSERT INTO "Marks_Entry_Report" 
                VALUES(p_MI_Id, p_ASMCL_Id, v_ASMS_Id_New, v_ASMCL_ClassName, v_ASMC_SectionName, 
                       v_ISMS_Id::BIGINT, v_ISMS_SubjectName, v_ISMS_order, v_employeename, 
                       v_enteryflag, v_enteryflag_color, v_SourceMarksEntry);
            END LOOP;
        END LOOP;
    ...
    RETURN QUERY 
    SELECT * FROM "Marks_Entry_Report" ORDER BY "SubjectOrder";
END;
$$;