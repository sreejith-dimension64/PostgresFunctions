...
BEGIN
    ...
    IF (p_typeflag = 'Overall') THEN
        IF p_optionflag = 'Item' THEN
            ...
            FROM (
                SELECT DISTINCT "MST"."INVMI_Id", "MI"."INVMI_ItemName", ("MST"."INVTSL_SalesQty"),
                ("MST"."INVTSL_SalesPrice"), ("INVTSL_DiscountAmt"),
                ("INVTSL_TaxAmt"), ("INVTSL_Amount"), ("INVMSL_SalesValue")
                FROM "INV"."INV_M_Sales" "MSL"
                LEFT JOIN "INV"."INV_T_Sales" "MST" ON "MST"."INVMSL_Id" = "MSL"."INVMSL_Id"
                LEFT JOIN "INV"."INV_Master_Item" "MI" ON "MI"."INVMI_Id" = "MST"."INVMI_Id"
        ...
        ELSIF p_optionflag = 'Student' THEN
            ...
            FROM (
                ...
                CASE WHEN COALESCE("AMST_MiddleName", '''') = '''' OR "AMST_MiddleName" = ''0'' THEN '''' ELSE '' '' || "AMST_MiddleName" END ||
                CASE WHEN COALESCE("AMST_LastName", '''') = '''' OR "AMST_LastName" = ''0'' THEN '''' ELSE '' '' || "AMST_LastName" END) AS "membername",
                "MI"."INVMI_Id",
    ...
    IF (p_typeflag = 'Detailed') THEN
        ...
        ELSIF p_optionflag = 'Saleno' THEN
            ...
            CASE WHEN COALESCE("AMST_MiddleName", '''') = '''' OR "AMST_MiddleName" = ''0'' THEN '''' ELSE '' '' || "AMST_MiddleName" END ||
            CASE WHEN COALESCE("AMST_LastName", '''') = '''' OR "AMST_LastName" = ''0'' THEN '''' ELSE '' '' || "AMST_LastName" END) AS "membername",
            "INVMSL_SalesDate", "MSL"."INVMSL_StuOtherFlg", 
