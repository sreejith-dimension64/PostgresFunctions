CREATE OR REPLACE FUNCTION "Exam_get_BB_Exam_Details_New"(
    "p_ASMAY_Id" bigint,
    "p_ASMCL_Id" bigint,
    "p_ASMS_Id" bigint,
    "p_MI_Id" bigint,
...
RETURNS TABLE(
    "AMST_Id" bigint,
    "AMST_FatherName" character varying,
    "AMST_MotherName" character varying,
    ...
    "SPCCMH_HouseName" character varying,
    ...
    "ESTMPS_ObtainedGrade" character varying,
    "ESTMPS_PassFailFlg" character varying,
    ...
    "ESTMP_TotalGrade" character varying,
    "ESTMP_ClassRank" integer,
    "ESTMP_SectionRank" integer,
    "ESTMP_TotalGradeRemark" character varying,
    ...
    "ESTMP_Result" character varying,
    "stu_grandmin_marks" numeric,
    "AMST_Photoname" character varying,
    "ESTMP_QRCode" character varying,
...
BEGIN
    ...
    IF "v_ExmConfig_LeftStuFlag" = false THEN
        ...
        SELECT DISTINCT 
            ...
            (SELECT SUM("asa_classheld") 
             ...
             WHERE "mi_id" = "p_MI_Id" AND "ASMAY_Id" = "p_ASMAY_Id"
                AND "ASMCL_Id" = "p_ASMCL_Id" AND "ASA_Activeflag" = 1 AND "ASMS_Id" = "p_ASMS_Id"
            ...
            s."ESTMP_Result",
            (SELECT COALESCE(SUM(Eyck."EYCES_MinMarks"), 0)
             FROM "Exm"."Exm_Category_Class" as sub
             INNER JOIN "Adm_School_Y_Student" std ON sub."ASMAY_Id" = std."ASMAY_Id" 
                AND sub."ASMCL_Id" = std."ASMCL_Id" AND sub."ASMS_Id" = std."ASMS_Id" 
                AND sub."ASMAY_Id" = "p_ASMAY_Id" AND std."ASMCL_Id" = "p_ASMCL_Id" 
                AND std."ASMS_Id" = "p_ASMS_Id"
             INNER JOIN "Exm"."Exm_Yearly_Category" exc ON sub."EMCA_Id" = exc."EMCA_Id" 
                AND exc."ASMAY_Id" = "p_ASMAY_Id" AND exc."EYC_ActiveFlg" = 1
             INNER JOIN "Exm"."Exm_Yearly_Category_Exams" as eyc ON eyc."EYC_Id" = exc."EYC_Id" 
                AND eyc."EME_Id" = "p_EME_Id" AND eyc."EYCE_ActiveFlg" = 1
             INNER JOIN "Exm"."Exm_Yrly_Cat_Exams_Subwise" as Eyck ON eyc."EYCE_Id" = Eyck."EYCE_Id" 
                AND Eyck."EYCES_AplResultFlg" = 1 AND eyck."EYCES_ActiveFlg" = 1
             WHERE std."AMST_Id" = f."AMST_Id" AND std."ASMAY_Id" = "p_ASMAY_Id"
                AND Eyck."ISMS_Id" IN (SELECT "ISMS_Id" FROM "Exm"."Exm_Student_Marks" 
                    WHERE "AMST_Id" = std."AMST_Id" AND "ASMAY_Id" = "p_ASMAY_Id" 
                    AND "EME_Id" = "p_EME_Id" AND "ASMS_Id" = "p_ASMS_Id" 
                    AND "ASMCL_Id" = "p_ASMCL_Id")
            ) as "stu_grandmin_marks",
            f."AMST_Photoname",
            s."ESTMP_QRCode",
        ...
        INNER JOIN "Exm"."Exm_Category_Class" as j ON j."ASMAY_Id" = "p_ASMAY_Id" 
            AND j."ASMCL_Id" = "p_ASMCL_Id" AND j."ASMS_Id" = "p_ASMS_Id" 
            AND j."MI_Id" = "p_MI_Id" AND j."ECAC_ActiveFlag" = 1
        INNER JOIN "Exm"."Exm_Yearly_Category" as k ON j."EMCA_Id" = k."EMCA_Id" 
            AND k."ASMAY_Id" = "p_ASMAY_Id" AND k."MI_Id" = "p_MI_Id" AND k."EYC_ActiveFlg" = 1
        INNER JOIN "Exm"."Exm_Yearly_Category_Exams" as l ON l."EYC_Id" = k."EYC_Id" 
            AND l."EME_Id" = "p_EME_Id" AND l."EYCE_ActiveFlg" = 1
        ...
        LEFT OUTER JOIN "Exm"."Exm_Student_Marks_Process" as s ON f."AMST_Id" = s."AMST_Id" 
            AND s."ASMCL_Id" = "p_ASMCL_Id" AND s."ASMS_Id" = "p_ASMS_Id" 
            AND s."ASMAY_Id" = "p_ASMAY_Id" AND s."MI_Id" = "p_MI_Id" AND s."EME_Id" = "p_EME_Id"
        LEFT OUTER JOIN "Exm"."Exm_Student_Marks_Process_Subjectwise" as a ON m."ISMS_Id" = a."ISMS_Id" 
            AND f."AMST_Id" = a."AMST_Id" AND a."ASMCL_Id" = "p_ASMCL_Id"
            AND a."ASMS_Id" = "p_ASMS_Id" AND a."ASMAY_Id" = "p_ASMAY_Id" 
            AND a."MI_Id" = "p_MI_Id" AND a."EME_Id" = "p_EME_Id"
        INNER JOIN "IVRM_Master_Subjects" as d ON m."ISMS_Id" = d."ISMS_Id" 
            AND d."MI_Id" = "p_MI_Id" AND m."ISMS_Id" = a."ISMS_Id"
        ...
        INNER JOIN "Exm"."Exm_Master_Exam" as c ON l."EME_Id" = c."EME_Id" 
            AND c."MI_Id" = "p_MI_Id" AND c."EME_ActiveFlag" = 1
        ...
        LEFT JOIN "SPC"."SPCC_Student_House" SPT ON spt."AMST_Id" = h."AMST_Id" 
            AND SPT."ASMAY_Id" = h."ASMAY_Id" AND SPT."ASMCL_Id" = h."ASMCL_Id" 
            AND SPT."ASMS_Id" = h."ASMS_Id" AND SPT."ASMAY_Id" = "p_ASMAY_Id"
            AND SPT."ASMCL_Id" = "p_ASMCL_Id" AND SPT."ASMS_Id" = "p_ASMS_Id"
        LEFT JOIN "SPC"."SPCC_Master_House" SPH ON SPH."SPCCMH_Id" = SPT."SPCCMH_Id" 
            AND SPH."MI_Id" = "p_MI_Id"
        WHERE h."AMST_Id" IN (SELECT UNNEST(string_to_array("p_AMST_Id", ','))::bigint)
        GROUP BY f."AMST_Id", f."AMST_FatherName", f."AMST_MotherName", f."AMST_FirstName", 
            f."AMST_MiddleName", f."AMST_LastName", f."AMST_DOB", h."AMAY_RollNo", f."AMST_AdmNo", 
            ...
            w."MI_name", m."EYCES_SubjectOrder", m."EYCES_MarksDisplayFlg", m."EYCES_GradeDisplayFlg", 
            s."ESTMP_Result", l."EYCE_AttendanceFromDate", l."EYCE_AttendanceToDate", 
            SPH."SPCCMH_HouseName", f."AMST_Photoname", s."ESTMP_QRCode", f."AMST_RegistrationNo"
