CREATE OR REPLACE FUNCTION "dbo"."ISM_Complaint_Task_Cart"(
    "RoleFlg" VARCHAR(100),
    "TypeFlg" VARCHAR(100),
    "status" VARCHAR(200),
    "HRME_Id" VARCHAR(100),
    ...
    IF "RoleFlg" = 'Staff' OR "RoleFlg" = 'Admin' OR "RoleFlg" = 'COORDINATOR' THEN
        IF "TypeFlg" = 'Default' THEN
            "Slqdymaic1" := 'CREATE TEMP TABLE "StaffAdmin_Temp1" AS SELECT DISTINCT TC."ISMTCR_Id",TC."HRMD_Id",MD."HRMD_DepartmentName",TC."HRMPR_Id",MP."HRMP_Name",
                ...
                (SELECT (COALESCE(ME."HRME_EmployeeFirstName",'''') || CASE WHEN COALESCE(ME."HRME_EmployeeMiddleName",''0'') = '''' OR COALESCE(ME."HRME_EmployeeMiddleName",''0'') = ''0'' THEN '''' ELSE '' '' || ME."HRME_EmployeeMiddleName" END || CASE WHEN COALESCE(ME."HRME_EmployeeLastName",''0'') = '''' OR COALESCE(ME."HRME_EmployeeLastName",''0'') = ''0'' THEN '''' ELSE '' '' || ME."HRME_EmployeeLastName" END)
            ...
            "Slqdymaic2" := 'CREATE TEMP TABLE "StaffAdmin_Temp2" AS SELECT DISTINCT TC."ISMTCR_Id",TC."HRMD_Id",MD."HRMD_DepartmentName",TC."HRMPR_Id",MP."HRMP_Name",
                ...
                AND TC."ISMTCR_Id" NOT IN (SELECT DISTINCT "ISMTCR_Id" FROM "ISM_TaskCreation_TransferredTo" WHERE "ISMTCRTRTO_ActiveFlg" = true)
                AND POSITION('','' || TC."ISMTCR_Status" || '','' IN '','' || ''' || "status" || ''' || '','') > 0
                AND CIE."ISMCIM_IEList" IN (' || "HRME_Id" || ') AND IRT."IVRMRT_RoleFlag" = ''ClientUser''';
            ...
            "Slqdymaic4" := 'CREATE TEMP TABLE "StaffAdmin_Temp4" AS SELECT DISTINCT TC."ISMTCR_Id",TC."HRMD_Id",MD."HRMD_DepartmentName",TC."HRMPR_Id",MP."HRMP_Name",
                ...
                (COALESCE(ME."HRME_EmployeeFirstName",'''') || CASE WHEN COALESCE(ME."HRME_EmployeeMiddleName",''0'') = '''' OR COALESCE(ME."HRME_EmployeeMiddleName",''0'') = ''0'' THEN '''' ELSE '' '' || ME."HRME_EmployeeMiddleName" END || CASE WHEN COALESCE(ME."HRME_EmployeeLastName",''0'') = '''' OR COALESCE(ME."HRME_EmployeeLastName",''0'') = ''0'' THEN '''' ELSE '' '' || ME."HRME_EmployeeLastName" END) AS assignedto,
