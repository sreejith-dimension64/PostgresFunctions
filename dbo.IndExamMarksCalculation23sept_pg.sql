CREATE OR REPLACE FUNCTION "dbo"."IndExamMarksCalculation23sept"(
    "p_MI_Id" bigint,
    "p_ASMAY_Id" bigint,
    "p_ASMCL_Id" bigint,
    "p_ASMS_Id" bigint,
...
BEGIN
    ...
    WHERE "MI_Id"="p_MI_Id" AND "ASMAY_Id"="p_ASMAY_Id" AND "ASMCL_Id"="p_ASMCL_Id" 
    AND "ASMS_Id"="p_ASMS_Id" AND "ECAC_ActiveFlag"=true LIMIT 1;

    SELECT "EYC_Id" INTO "v_EYC_Id" 
    FROM "Exm"."Exm_Yearly_Category" 
    WHERE "MI_Id"="p_MI_Id" AND "ASMAY_Id"="p_ASMAY_Id" AND "EMCA_Id"="v_EMCA_Id" 
    AND "EYC_ActiveFlg"=true LIMIT 1;

    DROP TABLE IF EXISTS "Exmsubjectdetails";

    SELECT "ExmConfig_RankingMethod", "ExmConfig_ClassRankFlg", "ExmConfig_SecRankFlg", 
           "ExmConfig_FailRankFlg", "ExmConfig_ClassPositionFlg", "ExmConfig_SectionPositionFlg"
    INTO "v_ExmConfig_RankingMethod", "v_ClassRankFlg", "v_SecRankFlg", "v_FailRankFlg", 
         "v_ClassPositionFlg", "v_SectionPositionFlg"
    ...
    SELECT DISTINCT "ESM"."AMST_Id", "ESM"."MI_Id", "ESM"."ASMAY_Id", "ESM"."ASMCL_Id", 
           "ESM"."ASMS_Id", "ESM"."ISMS_Id", "ESM"."EME_Id", "EYCES"."EYCES_AplResultFlg", 
           "EYCES"."EYCES_MarksEntryMax", "EYCES"."EYCES_MaxMarks", "EYCES"."EYCES_MinMarks", 
           "EYCES"."EMGR_Id", "ESM"."ESTM_Marks", "ESM"."ESTM_MarksGradeFlg", "ESM"."ESTM_Grade", "ESM"."ESTM_Flg"
    ...
    INNER JOIN "Exm"."Exm_Category_Class" AS "ECC" ON "ECC"."MI_Id"="p_MI_Id" AND "ECC"."ASMAY_Id"="p_ASMAY_Id"
    ...
    INNER JOIN "Exm"."Exm_Student_Marks" AS "ESM" ON "ESM"."AMST_Id" = "f"."AMST_Id" 
        ...
        AND "ESM"."ASMAY_Id"="h"."ASMAY_Id" AND "ESM"."ASMS_Id"="h"."ASMS_Id" 
        AND "ESM"."ASMCL_Id"="h"."ASMCL_Id" AND "ESM"."EME_Id"="p_EME_Id" 
        AND "ECC"."EMCA_Id"="v_EMCA_Id"
    ...
    FOR "rec_exmsubject" IN SELECT * FROM "Exmsubjectdetails" LOOP
        ...
        IF "v_ESTM_MarksGradeFlg"='M' THEN
            ...
            FROM "Exm"."Exm_Master_Grade_Details" 
            WHERE("v_GradeMarksPercentage" BETWEEN "EMGD_From" AND "EMGD_To") 
            AND "EMGR_Id"="v_EMGR_Id" LIMIT 1;
        ELSIF "v_ESTM_MarksGradeFlg"='G' THEN
        ...
        INSERT INTO "Exm"."Exm_Student_Marks_Process_Subjectwise" (
            ...
            "p_MI_Id", "p_ASMAY_Id", "p_ASMCL_Id", "p_ASMS_Id", "v_AMST_Id", "v_ISMS_Id", 
        ...
        INNER JOIN "Exm"."Exm_Yearly_Category_Exams" "c" ON "c"."EYC_Id"="b"."EYC_Id" 
            AND "c"."EYCE_ActiveFlg"=true AND "c"."EYC_Id"="v_EYC_Id"
        ...
        WHERE "a"."MI_Id"="p_MI_Id" AND "a"."ASMAY_Id"="p_ASMAY_id" 
            ...
            AND "a"."EMCA_Id"="v_EMCA_Id" LIMIT 1;
        ...
        IF ("v_EYCES_SubExamFlg"=false AND "v_EYCES_SubSubjectFlg"=true) 
           ...
           OR ("v_EYCES_SubExamFlg"=true AND "v_EYCES_SubSubjectFlg"=true) THEN
            ...
            SELECT DISTINCT "ESTMPS_Id" INTO "v_ESTMPS_Id_U"
            FROM "Exm"."Exm_Student_Marks_Pro_Sub_SubSubject" "SS" 
            WHERE "ESTMPSSS_PassFailFlg"='Fail' 
            AND "EMSS_Id" IN (
                SELECT "EMSS_Id" FROM "Exm"."Exm_Yrly_Cat_Exams_Subwise_SubSubjects" 
                WHERE "EYCESSS_AplResultFlg"=true AND "EYCESSS_ActiveFlg"=true
            )
            AND "ESTMPS_Id" IN (
                SELECT DISTINCT "ESTMPS_Id" 
                FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
                WHERE "AMST_Id"="v_AMST_Id" AND "MI_Id"="p_MI_Id" 
                AND "ASMAY_Id"="p_ASMAY_Id" AND "ASMCL_Id"="p_ASMCL_Id" 
                AND "ASMS_Id"="p_ASMS_Id" AND "EME_Id"="p_EME_Id" 
                AND "ISMS_Id"="v_ISMS_Id" AND "ESTMPS_PassFailFlg"='Pass'
            ) LIMIT 1;

            UPDATE "Exm"."Exm_Student_Marks_Process_Subjectwise" 
            SET "ESTMPS_PassFailFlg"='Pass'
            WHERE "ESTMPS_Id"="v_ESTMPS_Id_U" AND "AMST_Id"="v_AMST_Id" 
    ...
    FOR "rec_examsubwise" IN 
        ...
        WHERE "EYC"."MI_Id"="p_MI_Id" AND "EYC"."ASMAY_Id"="p_ASMAY_Id" 
            AND "ECC"."ASMCL_Id"="p_ASMCL_Id" AND "ECC"."ASMS_Id"="p_ASMS_Id" 
            AND "EYCE"."EME_Id"="p_EME_Id" AND "ECC"."EMCA_