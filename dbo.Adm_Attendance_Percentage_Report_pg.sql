CREATE OR REPLACE FUNCTION "dbo"."Adm_Attendance_Percentage_Report"(
    "MI_Id" TEXT,
    "ASMAY_Id" TEXT,
    "ASMCL_Id" TEXT,
    "ASMS_Id" TEXT,
...
BEGIN
    ...
    SELECT c."AMST_Id",
           "AMST_AdmNo",
           ("AMST_FirstName" || '' '' || "AMST_MiddleName" || '' '' || "AMST_LastName") AS studentname,
           ...
           CAST((SUM("ASA_Class_Attended")*100.0/SUM("ASA_ClassHeld")) AS NUMERIC(18,2)) AS percentage
    ...
    WHERE a."MI_Id" = ' || "MI_Id" || ' 
          ...
          AND ("ASA_FromDate" BETWEEN ''' || "fromdate" || ''' AND ''' || "todate" || ''') 
          AND a."ASMCL_Id" = ' || "ASMCL_Id" || ' 
          AND c."ASMCL_Id" = ' || "ASMCL_Id" || ' 
          AND A."ASMS_Id" IN (' || "ASMS_Id" || ') 
          AND C."ASMS_Id" IN (' || "ASMS_Id" || ')
    GROUP BY c."AMST_Id", "AMST_AdmNo", "AMST_FirstName", "AMST_MiddleName", "AMST_LastName", 
             "AMAY_RollNo", "ASMCL_ClassName", "ASMC_SectionName", "asmcl_order", "AMST_MobileNo", 
             "AMST_emailId", "asmc_order"
