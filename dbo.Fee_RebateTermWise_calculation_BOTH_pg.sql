CREATE OR REPLACE FUNCTION "Fee_RebateTermWise_calculation_BOTH"(
    "p_MI_Id" VARCHAR(500),
...
BEGIN
    ...
    WHERE "MI_ID" = "p_MI_Id" AND "USERID" = "p_USERID" AND "ASMAY_ID" != 0;
    ...
    WHERE "MI_ID" = "p_MI_Id" AND "ASMAY_Id" = "p_ASMAY_ID" AND "FYREBSET_ActiveId" = 1;
    ...
    WHERE "Amst_Id" IN (SELECT DISTINCT "Amst_Id" FROM "Adm_School_Y_Student" WHERE "amst_id" = "p_AMST_ID"::BIGINT AND "asmay_id" = "p_ASMAY_ID"::BIGINT)
    ...
    EXECUTE format('
        ...
        INNER JOIN "Fee_Group_Login_Previledge" ON "Fee_Group_Login_Previledge"."fmg_id" = "Fee_Student_Status"."FMG_Id"
            AND "Fee_Group_Login_Previledge"."fmh_id" = "Fee_Student_Status"."fmh_id"
        WHERE "Amst_Id" IN (SELECT DISTINCT "Amst_Id" FROM "Adm_School_Y_Student" WHERE "amst_id" = %s AND "asmay_id" = %s)
        AND "FSS_ActiveFlag" = 1 
        ...
        "p_AMST_ID", "p_ASMAY_ID", "p_MI_Id", "p_ASMAY_ID", "p_USERID", "p_FMT_ID");
    ...
    EXECUTE format('
        ...
        "p_AMST_ID", "p_ASMAY_ID", "p_MI_Id", "p_ASMAY_ID", "p_USERID");
    ...
    IF ("v_FMC_RebateAplicableFlg" = TRUE AND "v_FMC_RebateAgainstFullPaymentFlg" = TRUE AND "v_FMC_RebateAgainstPartialPaymentFlg" = TRUE) THEN
        ...
        IF ("p_paidamount" >= "v_totalyrcharges") THEN
            ...
            LOOP
                "v_FMT_IDFullpayment" := "v_rec"."FMT_ID";
                ...
                WHERE "MI_ID" = "p_MI_Id" AND "ASMAY_Id" = "p_ASMAY_ID" AND "FMG_ID" = "v_FMG_IDFullpayment";
        ...
        ELSIF ("p_paidamount" < "v_totalyrcharges") THEN
            ...
            FOR "v_rec" IN 
                SELECT * FROM "groupheadinstallment_temp"
            LOOP
                "v_FMT_IDPartialpercent" := "v_rec"."FMT_ID";
                "v_FMG_IDPartialpercent" := "v_rec"."fmg_id";
                "v_FMH_IDPartialpercent" := "v_rec"."Fmh_id";
                "v_FTI_IDPartialpercent" := "v_rec"."fti_id";

                SELECT "FYG_RebateApplicableFlg"
                INTO "v_FYGH_RebateApplicableFlg"
                FROM "Fee_Yearly_Group"
                WHERE "MI_ID" = "p_MI_Id" AND "ASMAY_Id" = "p_ASMAY_ID" AND "FMG_ID" = "v_FMG_IDPartialpercent";
                ...
                IF ("v_FYGH_RebateApplicableFlg" = TRUE) THEN
                    ...
                    WHERE "FMT_Id" = "v_FMT_IDPartialpercent" AND "ASMAY_ID" = "p_ASMAY_ID";
                    ...
                    IF ("v_FMTRS_RebateAmountPercentFlg" = 'Percentage') THEN
                        ...
                        FROM "Fee_Student_Status"
                        WHERE "MI_ID" = "p_MI_Id" AND "ASMAY_Id" = "p_ASMAY_ID" AND "FMG_ID" = "v_FMG_IDPartialpercent"
                        AND "FMH_Id" = "v_FMH_IDPartialpercent" AND "FTI_Id" = "v_FTI_IDPartialpercent" AND "AMST_ID" = "p_AMST_ID"::BIGINT;
            ...
            IF ("v_FMTRS_RebateAmountPercentFlg" = 'Amount') THEN
                ...
                LOOP
                    "v_FMT_IDPartialfmtid" := "v_rec"."FMT_ID";
                    ...
                    INTO "v_FMTRS_RebateApplicableDate", "v_FMTRS_RebateAmountPercentFlg", "v_FMTRS_RebateAmountPercentValue"
                    FROM "Fee_Master_Terms_RebateSetting"
                    WHERE "FMT_Id" = "v_FMT_IDPartialfmtid" AND "ASMAY_ID" = "p_ASMAY_ID";
    ...
    DROP TABLE IF EXISTS "currentyrchargestotal";
    DROP TABLE IF EXISTS "groupcount";
    DROP TABLE IF EXISTS "#Termids_temp";
