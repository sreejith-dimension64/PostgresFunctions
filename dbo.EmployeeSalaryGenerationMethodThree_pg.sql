CREATE OR REPLACE FUNCTION "dbo"."EmployeeSalaryGenerationMethodThree"(
    p_HRME_ID BIGINT,
    p_MI_ID BIGINT,
    p_HRMLY_ID BIGINT,
...
BEGIN
    SELECT "MI_ID", "HRME_Id", "HRMET_Id", "HRMGT_Id", "HRMD_Id",
           "HRMDES_Id", "HRMG_Id", "HRME_PFApplicableFlag", "HRME_PFDate", "HRME_PFFixedFlag",
    ...
    INTO v_EMPMI_ID, v_EMPHRME_ID, v_EMPHRMET_Id, v_EMPHRMGT_Id, v_EMPHRMD_Id,
         v_EMPHRMDES_Id, v_EMPHRMG_Id, v_HRME_PFApplicableFlag, v_HRME_PFDate, v_HRME_PFFixedFlag,
    ...
    IF (CAST(v_MONTH_START_DATE AS DATE) > CAST(CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-', v_DAYFROM) AS DATE)) THEN
        v_EMPTOTAL_DAYS := v_DAYS_IN_MONTH;
        v_LOPDAYS := v_DAYS_IN_MONTH - (v_DAYS_IN_MONTH - EXTRACT(DAY FROM CAST(v_MONTH_START_DATE AS DATE)) + 1 - v_LOPDAYS);
    END IF;
    ...
    LOOP
        ...
        IF v_TEMPSALCOUNT = 0 THEN
            IF v_HRMED_EDTypeFlagCUR1 = 'Basic Pay' THEN
                RAISE NOTICE 'INSERT % >> Basic Pay Already exist', v_HRMED_IdCUR1;
            ELSE
                ...
                RAISE NOTICE 'INSERT >> %', v_HRMED_IdCUR1;
            END IF;
        END IF;
    END LOOP;
