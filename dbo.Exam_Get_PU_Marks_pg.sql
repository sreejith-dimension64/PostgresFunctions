CREATE OR REPLACE FUNCTION "dbo"."Exam_Get_PU_Marks"(
    p_ASMAY_Id bigint,
    p_ASMCL_Id bigint,
    p_ASMS_Id bigint,
    p_MI_Id bigint,
...
BEGIN
    ...
    SELECT DISTINCT 
        ...
        (SELECT sum("ASA_Class_Attended") FROM "Adm_Student_Attendance_Students" q, "Adm_Student_Attendance" p
         ...
         AND "ASMCL_Id"=p_ASMCL_Id AND "ASMS_Id"=p_ASMS_Id AND q."AMST_Id"=f."AMST_Id" 
         AND ((TO_DATE(p."ASA_FromDate",'DD/MM/YYYY') BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate") 
    ...
    GROUP BY f."AMST_Id", f."AMST_FirstName", f."AMST_MiddleName", f."AMST_LastName", f."AMST_DOB", 
        ...
        b."ASMCL_ClassName", e."ASMC_SectionName", r."EMGD_Remarks", s."ESTMP_TotalObtMarks", 
        s."ESTMP_Percentage", s."ESTMP_TotalGrade", s."ESTMP_ClassRank", s."ESTMP_SectionRank", 
        t."EMGD_Remarks", s."ESTMP_TotalMaxMarks", w."MI_name", m."EYCES_SubjectOrder", 
        m."EYCES_MarksDisplayFlg", m."EYCES_GradeDisplayFlg", s."ESTMP_Result", 
