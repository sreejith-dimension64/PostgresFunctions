CREATE OR REPLACE FUNCTION "dbo"."Get_Student_Status_AdvanceDetails"(
    "p_MI_Id" VARCHAR(10),
...
BEGIN
    ...
    IF "v_Rcount" > 0 THEN
        ...
        'SELECT DISTINCT 
            ...
            0::NUMERIC AS "FCSS_ConcessionAmount",
            "FCMAS"."FCMAS_Amount" AS "FCSS_NetAmount",
            0::NUMERIC AS "FCSS_FineAmount",
            0::NUMERIC AS "FCSS_RefundAmount",
            "FMH"."FMH_FeeName",
            "FTI"."FTI_Name",
            "FMG"."FMG_GroupName",
            "FCMAS"."FCMAS_Amount" AS "FCSS_CurrentYrCharges",
            "FCMAS"."FCMAS_Amount" AS "FCSS_TotalCharges",
            0::NUMERIC AS "FCSS_OBArrearAmount",
            0::NUMERIC AS "FCSS_WaivedAmount",
            "FMH"."FMH_Order",
            "FMH"."FMH_Flag",
            "FCMAS"."FCMAS_DueDate" AS "FCMAS_DueDate",
            "AMS"."AMSE_SEMName",
            (CASE WHEN CURRENT_DATE > MAKE_DATE("TDD"."FCTDD_Year", "TDD"."FCTDD_Month", "TDD"."FCTDD_Day") 
                THEN EXTRACT(DAY FROM CURRENT_DATE - MAKE_DATE("TDD"."FCTDD_Year", "TDD"."FCTDD_Month", "TDD"."FCTDD_Day"))::INTEGER + 1 
                ELSE 0 
            END) AS "DiffDays"
        ...
        LEFT JOIN "Clg_StudentAdvancePaidA_temp" "AT" 
            ON "AT"."FMG_Id" = "FCMA"."FMG_Id" 
            ...
            AND "FCMAS"."AMSE_Id" IN (
                ...
                WHERE "MI_Id" = ' || "p_MI_Id" || ' 
                    AND "AMSE_SemOrder" > (
                        SELECT "AMSE_SemOrder" 
                        FROM "clg"."Adm_Master_Semester" 
                        WHERE "MI_Id" = ' || "p_MI_Id" || ' AND "AMSE_Id" = ' || "v_AMSE_Id" || '
                    ) 
                ORDER BY "AMSE_SemOrder" 
                LIMIT 100
            )
