CREATE OR REPLACE FUNCTION "dbo"."CLG_Auto_Fee_Group_Mapping_Hostel_Request"(
    p_MI_Id bigint,
    p_ASMAY_Id bigint,
    p_AMCST_Id bigint,
    p_UserId bigint,
    p_EntireRoomReqdFlg boolean
)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_ACCount int;
    v_FMC_CommonHostelFeeFlg boolean;
...
BEGIN
    ...
    BEGIN
        ...
        IF v_SFS = 0 OR v_SFS IS NULL THEN
            DROP TABLE IF EXISTS "TempGroupHostelRQ_College";

            CREATE TEMP TABLE "TempGroupHostelRQ_College" AS
            ...
            AND "FMG_HostelFlg" = 1;
        ELSE
            DELETE FROM "TempGroupHostelRQ_College";
        END IF;

        FOR rec_yearly_fee IN 
            SELECT "FMG_Id" FROM "TempGroupHostelRQ_College"
            UNION 
        ...
        LOOP
            ...
            WHERE "FMG_Id" = v_FMG_Id AND "MI_Id" = p_MI_Id AND "AMCST_Id" = p_AMCST_Id 
            AND "ASMAY_Id" = p_ASMAY_Id AND "FCMSGH_ActiveFlag" = 1;
            ...
            FROM "CLG"."Fee_College_Master_Student_GroupHead" 
            WHERE "AMCST_Id" = p_AMCST_Id AND "ASMAY_Id" = p_ASMAY_Id AND "FMG_Id" = v_FMG_Id AND "FCMSGH_ActiveFlag" = 1;
            ...
            SELECT DISTINCT a."AMCO_Id", a."AMB_Id" INTO v_AMCO_Id, v_AMB_Id 
            FROM "CLG"."Adm_College_Yearly_Student" a 
            INNER JOIN "CLG"."Adm_Master_College_Student" b ON a."AMCST_Id" = b."AMCST_Id" 
            ...
            ELSE
                ...
                FROM "CLG"."Adm_Master_College_Student" 
                WHERE "AMCST_Id" = p_AMCST_Id AND "MI_Id" = p_MI_Id AND "ASMAY_Id" = p_ASMAY_Id;
            END IF;
            ...
            SELECT COALESCE("FMC_CommonHostelFeeFlg", false) INTO v_FMC_CommonHostelFeeFlg 
            FROM "Fee_Master_Configuration" 
            WHERE "MI_Id" = p_MI_Id;

            DROP TABLE IF EXISTS "Fee_CollegeSemesterWiseAmount_hostelRQ_Temp";

            IF (v_FMG_BatchwiseFeeApplFlg = true) AND v_FMC_CommonHostelFeeFlg = true THEN
                CREATE TEMP TABLE "Fee_CollegeSemesterWiseAmount_hostelRQ_Temp" AS
                SELECT "FA"."FMH_Id", "FTI_Id", "FS"."FCMAS_Id", "FCMAS_Amount"
                FROM "CLG"."Fee_College_Master_Amount" "FA"
                INNER JOIN "CLG"."Fee_College_Master_Amount_Semesterwise" "FS" ON "FA"."FCMA_Id" = "FS"."FCMA_Id" 
                    AND "FA"."AMCO_Id" = v_AMCO_Id AND "FA"."AMB_Id" = v_AMB_Id AND "FS"."AMSE_Id" = v_AMSE_Id
                INNER JOIN "Fee_Master_Head" "FMH" ON "FMH"."FMH_Id" = "FA"."FMH_Id" AND "FMH"."FMH_Flag" = 'H'
                INNER JOIN "Fee_Master_Group" "FMG" ON "FMG"."FMG_Id" = "FA"."FMG_Id" AND "FMG"."FMG_HostelFlg" = 1
                WHERE "FA"."FMG_Id" = v_FMG_Id AND "ASMAY_Id" = v_JASMAY_Id AND "FS"."MI_Id" = p_MI_Id AND "FA"."MI_Id" = p_MI_Id;
            ELSIF (v_FMG_BatchwiseFeeApplFlg = false) AND v_FMC_CommonHostelFeeFlg = true THEN
                CREATE TEMP TABLE "Fee_CollegeSemesterWiseAmount_hostelRQ_Temp" AS
                SELECT "FA"."FMH_Id", "FTI_Id", "FS"."FCMAS_Id", "FCMAS_Amount"
                FROM "CLG"."Fee_College_Master_Amount" "FA"
                INNER JOIN "CLG"."Fee_College_Master_Amount_Semesterwise" "FS" ON "FA"."FCMA_Id" = "FS"."FCMA_Id" 
                    AND "FA"."AMCO_Id" = v_AMCO_Id AND "FA"."AMB_Id" = v_AMB_Id AND "FS"."AMSE_Id" = v_AMSE_Id
                INNER JOIN "Fee_Master_Head" "FMH" ON "FMH"."FMH_Id" = "FA"."FMH_Id" AND "FMH"."FMH_Flag" = 'H'
                WHERE "FA"."FMG_Id" = v_FMG_Id AND "ASMAY_Id" = p_ASMAY_Id AND "FS"."MI_Id" = p_MI_Id AND "FA"."MI_Id" = p_MI_Id;
            END IF;

            FOR rec_instidssem IN 
                SELECT "FMH_Id", "FTI_Id", "FCMAS_Id", "FCMAS_Amount" 
                FROM "Fee_CollegeSemesterWiseAmount_hostelRQ_Temp"
            LOOP
                ...
                IF (p_EntireRoomReqdFlg = true) AND v_FMC_CommonHostelFeeFlg = true THEN
                    SELECT "HLMRCA_RoomRate" INTO v_FCMAS_Amount 
                    FROM "HL_Master_Room_Category" "RC"
                    INNER JOIN "HL_Master_Room" "HMR" ON "HMR"."HLMRCA_Id" = "RC"."HLMRCA_Id"
                    INNER JOIN "HL_Master_Room_FeeGroup" "MRG" ON "MRG"."HRMRM_Id" = "HMR"."HRMRM_Id"
                    WHERE "MRG"."MI_Id" = p_MI_Id AND "MRG"."FMG_Id" = v_FMG_Id;
                ELSIF (p_EntireRoomReqdFlg = false) AND v_FMC_CommonHostelFeeFlg = true THEN
                    SELECT "HLMRCA_SORate" INTO v_FCMAS_Amount 
                    FROM "HL_Master_Room_Category" "RC"
                    INNER JOIN "HL_Master_Room" "HMR" ON "HMR"."HLMRCA_Id" = "RC"."HLMRCA_Id"
                    INNER JOIN "HL_Master_Room_FeeGroup" "MRG" ON "MRG"."HRMRM_Id" = "HMR"."HRMRM_Id"
                    WHERE "MRG"."MI_Id" = p_MI_Id AND "MRG"."FMG_Id" = v_FMG_Id;
                ...
                IF (v_FCSSRcount = 0) THEN
                    INSERT INTO "CLG"."Fee_College_Student_Status"(
                        ...
                        "FCSS_OBArrearAmount", "FCSS_OBExcessAmount", "FCSS_CurrentYrCharges", "FCSS_TotalCharges", 
                        "FCSS_ToBePaid", "FCSS_PaidAmount", "FCSS_ExcessPaidAmount", "FCSS_ExcessAmountAdjusted", 
