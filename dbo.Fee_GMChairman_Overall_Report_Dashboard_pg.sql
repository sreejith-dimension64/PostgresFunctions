CREATE OR REPLACE FUNCTION dbo."Fee_GMChairman_Overall_Report_Dashboard"(
    p_ASMAY_Id bigint,
    p_userId bigint
)
RETURNS TABLE (
...
BEGIN
    ...
    INNER JOIN dbo."IVRM_User_Login_Institutionwise" b
        ON a."MI_Id" = b."MI_Id"
    WHERE b."Id" = p_userId AND a."ASMAY_Id" = p_ASMAY_Id;
    ...
    INNER JOIN temp_GMFEE "Y" 
        ON "Y"."mi_id" = "FSS"."MI_Id"  
        AND "FSS"."ASMAY_Id" = "Y"."ASMAY_Id"
    INNER JOIN dbo."Adm_School_Y_Student" 
        ON "FSS"."Amst_Id" = dbo."Adm_School_Y_Student"."AMST_Id" 
    ...
    INNER JOIN dbo."master_institution" "MI" 
        ON "MI"."MI_ID" = "FSS"."MI_ID"
    INNER JOIN dbo."Fee_Master_Group" 
        ON "FSS"."fmg_id" = dbo."Fee_Master_Group"."FMG_Id"
    INNER JOIN dbo."Fee_Master_Head" 
        ON dbo."Fee_Master_Head"."FMH_Id" = "FSS"."FMH_Id"
    INNER JOIN dbo."Fee_T_Installment" 
        ON dbo."Fee_T_Installment"."FTI_Id" = "FSS"."FTI_Id"
    INNER JOIN dbo."Fee_Master_Terms_FeeHeads" 
        ON dbo."Fee_Master_Terms_FeeHeads"."FMH_Id" = "FSS"."FMH_Id" 
        AND dbo."Fee_Master_Terms_FeeHeads"."FTI_Id" = "FSS"."FTI_Id"
    WHERE "FSS"."AMST_Id" IN (
        SELECT b."AMST_Id" 
        FROM dbo."Adm_M_Student" a
        INNER JOIN dbo."Adm_school_y_student" b 
            ON b."amst_id" = a."amst_id"
        WHERE a."MI_Id" IN (SELECT "mi_id" FROM temp_GMFEE) 
