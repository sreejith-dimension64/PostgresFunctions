CREATE OR REPLACE FUNCTION "dbo"."Exam_cumulative_Vikasa_Report"(
    p_ASMAY_Id bigint,
    p_ASMCL_Id bigint,
    p_ASMS_Id bigint,
    p_MI_Id bigint,
...
BEGIN
        ...
        (SELECT SUM(asa_classheld) 
         ...
         WHERE "mi_id" = p_MI_Id 
           ...
           AND ((TO_DATE(p."ASA_FromDate"::TEXT, 'DD/MM/YYYY') BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate") 
                OR (TO_DATE(p."ASA_ToDate"::TEXT, 'DD/MM/YYYY') BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate"))
        ) AS "ASA_ClassHeld",
    ...
    INNER JOIN "Adm_School_Y_Student" AS h 
        ON h."AMST_Id" = f."AMST_Id" 
    ...
    INNER JOIN "Exm"."Exm_Category_Class" AS j 
        ON j."ASMAY_Id" = p_ASMAY_Id 
        AND j."ASMCL_Id" = p_ASMCL_Id 
        AND j."ASMS_Id" = p_ASMS_Id 
        AND j."MI_Id" = p_MI_Id
    INNER JOIN "Exm"."Exm_Yearly_Category" AS k 
        ON j."EMCA_Id" = k."EMCA_Id" 
        AND k."ASMAY_Id" = p_ASMAY_Id 
        AND k."MI_Id" = p_MI_Id
    INNER JOIN "Exm"."Exm_Yearly_Category_Exams" AS l 
        ON l."EYC_Id" = k."EYC_Id" 
        AND l."EME_Id" = p_EME_Id 
    INNER JOIN "Exm"."Exm_Yrly_Cat_Exams_Subwise" AS m  
        ON m."EYCE_Id" = l."EYCE_Id" 
        AND m."EYCES_ActiveFlg" = 1 
        AND m."EYCES_AplResultFlg" = 1
    LEFT OUTER JOIN "Exm"."Exm_Student_Marks_Process" AS s 
        ON f."AMST_Id" = s."AMST_Id"  
