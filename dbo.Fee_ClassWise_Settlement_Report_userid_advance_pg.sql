CREATE OR REPLACE FUNCTION "dbo"."Fee_ClassWise_Settlement_Report_userid_advance" (
    "p_MI_Id" BIGINT,
    "p_ASMAY_Id" BIGINT,
    "p_Fromdate" TEXT,
    "p_Todate" TEXT,
...
BEGIN
    ...
    SELECT "dbo"."Adm_m_student"."AMST_Id" AS "AMST_Id",
           ...
           "dbo"."Fee_Master_Head"."FMH_FeeName" AS "FeeName",
    ...
    WHERE "Fee_Y_Payment"."mi_id" = ' || "p_MI_Id" || ' AND "Fee_Y_Payment"."user_Id" IN (' || "p_userid" || ') ' || "v_where_condition";

    EXECUTE "v_sqlquery";

    IF "v_sqlquery1" IS NOT NULL THEN
        EXECUTE "v_sqlquery1";
    END IF;
    ...
    SELECT STRING_AGG(DISTINCT '"' || "FeeName" || '"', ',') INTO "v_PivotColumnNames"
    FROM "FeeHead_Temp";

    SELECT STRING_AGG(DISTINCT 'COALESCE("' || "FeeName" || '", 0) AS "' || "FeeName" || '"', ',') INTO "v_PivotSelectColumnNames"
    FROM "FeeHead_Temp";
    ...
    "v_SQLQuery2" := 'SELECT "AMST_Id", "StudentName", "ClassName", "SectionName", "AdmNo", "UTR_No", "Transactionid", "PaymentId", 
                            ...
                            TO_CHAR("FYP_Date", ''DD/MM/YYYY'') AS "FYP_Date",
                            ROW_TO_JSON(fee_data) AS "DynamicColumns"
                     FROM (
                         SELECT "AMST_Id", "StudentName", "ClassName", "SectionName", "AdmNo", "UTR_No", "Transactionid", "PaymentId",
                                "FYPPST_Settlement_Date", "FYP_Date", ' || "v_PivotSelectColumnNames" || '
                         FROM CROSSTAB(
                             ''SELECT "AMST_Id"::TEXT || ''|'' || "StudentName" || ''|'' || "ClassName" || ''|'' || "SectionName" || ''|'' || "AdmNo" || ''|'' || "UTR_No" || ''|'' || "Transactionid" || ''|'' || "PaymentId"::TEXT || ''|'' || "FYPPST_Settlement_Date"::TEXT || ''|'' || "FYP_Date"::TEXT AS composite_key, 
                                     "FeeName", 
                                     SUM("paidAmount") 
                              FROM "FeeSettementReport_New" 
                              GROUP BY composite_key, "FeeName" 
                              ORDER BY 1, 2'',
                             ''SELECT DISTINCT "FeeName" FROM "FeeHead_Temp" ORDER BY 1''
                         ) AS ct(composite_key TEXT, ' || "v_PivotColumnNames" || ')
                     ) AS pivot_result
                     CROSS JOIN LATERAL (
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error in Fee_ClassWise_Settlement_Report_userid_advance: %', SQLERRM;
END;
$$;