CREATE OR REPLACE FUNCTION "dbo"."Get_Student_Status_AdvanceDetailsAdvance"(
    p_MI_Id VARCHAR(10),
...
BEGIN
    ...
    DROP TABLE IF EXISTS "FeeBatchWise_Tempadvance";
    
    v_SqlFeegroupDynamic := 
        'CREATE TEMP TABLE "FeeBatchWise_Tempadvance" AS
        SELECT DISTINCT COALESCE("FMG"."FMG_BatchwiseFeeApplFlg", false) AS "FMG_BatchwiseFeeApplFlg"
        FROM "Fee_Master_Group" "FMG"
        INNER JOIN "Fee_Yearly_Group" "FYG" ON "FMG"."FMG_Id" = "FYG"."FMG_Id"
        WHERE "FYG"."ASMAY_Id" = ' || p_ASMAY_Id || ' 
        AND "FMG"."MI_Id" = ' || p_MI_Id || ' 
        AND "FMG"."FMG_Id" IN (' || p_FMG_Id || ')';
    
    EXECUTE v_SqlFeegroupDynamic;
    
    SELECT "FMG_BatchwiseFeeApplFlg" INTO v_FMG_BatchwiseFeeApplFlg 
    FROM "FeeBatchWise_Tempadvance" 
    LIMIT 1;
    ...
    IF (v_Rcount > 0) THEN
        v_query := 
            'SELECT DISTINCT "FCMA"."FMG_Id", "FCMAS"."FCMAS_Id", "FCMA"."FMH_Id", "FCMA"."FTI_Id", "FCMA"."ASMAY_Id", 
                    ...
                    "FMH"."FMH_Order", "FMH"."FMH_Flag", "FCMAS"."FCMAS_DueDate"
            ...
            INNER JOIN "Fee_T_Installment" "FTI" ON "FTI"."FTI_Id" = "FCMA"."FTI_Id" 
                AND "FTI"."MI_Id" = ' || p_MI_Id || '
            INNER JOIN "Fee_Master_Group" "FMG" ON "FMG"."FMG_Id" = "FCMA"."FMG_Id" 
                AND "FMG"."MI_Id" = ' || p_MI_Id || '
            INNER JOIN "Fee_Group_Login_Previledge" "LP" ON "LP"."FMG_Id" = "FMG"."FMG_Id" 
                AND "LP"."MI_Id" = ' || p_MI_Id || '
