CREATE OR REPLACE FUNCTION "dbo"."EmployeeSalaryDetailDynamicCalculation"(
    p_MI_ID BIGINT,
    p_HRME_ID BIGINT
)
RETURNS TABLE (
    ...
    FOR rec IN
        ...
        WHERE A."MI_Id" = p_MI_ID AND B."HRME_Id" = p_HRME_ID 
          ...
          AND A."HRMED_Id" NOT IN (
              SELECT "hrmed_id" FROM "temp_salary_comp_detail" 
        ...
        IF v_TEMPSALCOUNT = 0 THEN
            FOR rec IN
                SELECT A."HRMED_Id", A."HRMEDP_HRMED_Id", B."hrmed_amount", C."HREED_Percentage", 
                       D."HRMED_EDTypeFlag", D."HRMED_EarnDedFlag", D."HRMED_AmountPercentFlag", D."HRMED_RoundOffFlag"
            ...
            LOOP
                ...
                IF v_CUR3HEADCOUNT = 0 THEN
                    ...
                    LOOP
                        ...
                        SELECT (("hrmed_amount" * v_HREED_PercentageCUR4::NUMERIC) / 100) INTO v_CALCULATEDHEADAMOUNT
                        FROM "temp_salary_comp_detail" 
                        WHERE "HRMED_Id" = v_HRMEDP_HRMED_IdCUR4;
                        
                        v_result3 := "dbo"."CalculateRoundOffValue"(v_CALCULATEDHEADAMOUNT, v_HRMED_RoundOffFlagCUR4);
                        v_CALCULATEDHEADAMOUNT := v_result3;
                        ...
                        v_result1 := "dbo"."CalculateRoundOffValue"(v_CALCULATEDHEADAMOUNT, v_HRMED_RoundOffFlagCUR3);
                        v_CALCULATEDHEADAMOUNT := v_result1;
                        
                        SELECT ((v_CALCULATEDHEADAMOUNT * v_HREED_PercentageCUR3::NUMERIC) / 100) INTO v_CALCULATEDHEADAMOUNT
                        FROM "temp_salary_comp_detail" 
                        WHERE "HRMED_Id" = v_HRMEDP_HRMED_IdCUR3;
                    ...
                    END LOOP;
                ELSE
                    SELECT (("hrmed_amount" * v_HREED_PercentageCUR3::NUMERIC) / 100) INTO v_CALCULATEDHEADAMOUNT
                    FROM "temp_salary_comp_detail" 
                    WHERE "HRMED_Id" = v_HRMEDP_HRMED_IdCUR3;
                    
                    v_result2 := "dbo"."CalculateRoundOffValue"(v_CALCULATEDHEADAMOUNT, v_HRMED_RoundOffFlagCUR3);
                    v_CALCULATEDHEADAMOUNT := v_result2;
