CREATE OR REPLACE FUNCTION "dbo"."EmployeeSalaryGenerationMethodSeven"(
    p_HRME_ID BIGINT,
    p_MI_ID BIGINT,
    p_HRMLY_ID BIGINT,
...
DECLARE
    ...
    v_HRME_LeftFlag BOOLEAN; v_EBasicRcount INT; v_ActiveEmpCount INT; v_ActualBasicHREED_Amount DECIMAL(18,2);
    v_ActualBasic DECIMAL(25,2);
    
    v_TOTAL_DAYS INT;
    v_EMPTOTAL_DAYS INT;
    
    v_IncentivesAmt DECIMAL(18,2);
    ...
    v_PT_HRMED_Id BIGINT; v_PT_HREED_Amount DECIMAL(18,2);
    v_CURPTHRMED_Id BIGINT; v_CURPTHRMEDP_HRMED_Id BIGINT;
    
    v_LocalConAmtPerDay DECIMAL(18,2); v_Rcount INT;
    ...
    v_CUR7HRMED_Id BIGINT; v_CUR7HRMED_AMOUNT DECIMAL(10,4); v_CUR7HRMED_EDTypeFlag VARCHAR(50); v_CUR7HRMED_EarnDedFlag VARCHAR(50);
    v_CUR7HRMED_RoundOffFlag VARCHAR(50); v_CUR7HRMED_amtperc_flag VARCHAR(50);
    
    v_result DECIMAL(10,4);
    v_SALARYPERDAYPF DECIMAL(10,4);
    
    v_result1 DECIMAL(18,2);
    v_result2 DECIMAL(18,2);
    v_result3 DECIMAL(18,2);
    
    v_EMP_ESIAmount DECIMAL(18,2); v_PerDayESI DECIMAL(18,2); v_EMP_ESIAmount_N DECIMAL(18,2);
    v_EmpGrossAmt DECIMAL(18,2); v_ESI_HREED_Percentage DECIMAL(18,2);
    v_EmpIncentivesAmt DECIMAL(18,2);
    
    v_HRESA_SanctinedAmount DECIMAL(18,2); v_MAX_SanctionLevelNo INT;
...
BEGIN
    ...
    IF v_ActiveEmpCount = 1 THEN
        ...
        SELECT "HRC_PFMaxAmt", "HRC_FPFPer", "HRC_EPFPer",
               ...
               "HRC_AC2MinAmount", "HRC_AC21MinAmount", "HRC_AC22MinAmount", 
        ...
        INTO v_HRC_PFMAXAMT, v_HRC_FPFPER, v_HRC_EPFPER,
             ...
             v_HRC_AC2MinAmount, v_HRC_AC21MinAmount, v_HRC_AC22MinAmount,
        ...
        ELSE
            v_MONTH_START_DATE := CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-', v_DAYFROM);
            v_DAYTO := EXTRACT(DAY FROM (DATE_TRUNC('MONTH', (v_LEAVE_YEAR || '-' || v_MONTH || '-01')::DATE) + INTERVAL '1 MONTH' - INTERVAL '1 DAY'));
            v_MONTH_END_DATE := CONCAT(v_LEAVE_YEAR, '-', v_MONTH, '-', v_DAYTO);
        END IF;
        
        RAISE NOTICE '%', v_MONTH_START_DATE;
        RAISE NOTICE '%', v_MONTH_END_DATE;
        ...
        SELECT COALESCE(SUM("HRELT_TotDays"), 0) INTO v_LOPDAYS
        ...
        AND A."HRELT_LeaveReason" = 'LWP Cumulative Report'
        ...
        RAISE NOTICE '@MONTH_START_DATE:%', v_MONTH_START_DATE;
        RAISE NOTICE '@MONTH_END_DATE: %', v_MONTH_END_DATE;
        RAISE NOTICE '@EMPTOTAL_DAYS-----------------: %', v_EMPTOTAL_DAYS;
        RAISE NOTICE '@LOPDAYS:%', v_LOPDAYS;
        ...
        WHERE "HRMED"."MI_Id" = p_MI_ID AND "HREED"."HRME_Id" = p_HRME_ID 
        AND "HRMED"."HRMED_EDTypeFlag" = 'Basic Pay'
        AND "HREED"."HREED_ActiveFlag" = TRUE AND "HRMED"."HRMED_ActiveFlag" = TRUE;
        
        RAISE NOTICE 'ActualBasicPay:%', v_BASICPAY;
        
        v_ActualBasic := v_BASICPAY;
        ...
        IF v_EMPTOTAL_DAYS < v_DAYS_IN_MONTH THEN
            ...
            RAISE NOTICE 'TotDays %', v_EMPTOTAL_DAYS;
        ...
        RAISE NOTICE 'RevisedBasicPay:%', v_BASICPAY;
        RAISE NOTICE 'Salaryperday:%', v_SALARYPERDAY;
        RAISE NOTICE '@EMPTOTAL_DAYS: %', v_EMPTOTAL_DAYS;
        RAISE NOTICE '@DAYS_IN_MONTH: %', v_DAYS_IN_MONTH;
        RAISE NOTICE 'LOPDAYS: %', v_LOPDAYS;
        RAISE NOTICE 'WORKINGDAYS: %', v_WORKINGDAYS;
        
        IF (v_HRME_DOJ BETWEEN v_MONTH_START_DATE::DATE AND v_MONTH_END_DATE::DATE) THEN
            v_BASICPAY := v_SALARYPERDAY * v_WORKINGDAYS;
        END IF;
        ...
        INSERT INTO "temp_salary_comp_detail" 
        VALUES (p_MI_ID, p_HRME_ID, v_HRMED_IdBasic, v_BASICPAY, v_HRMED_EDTypeFlagBasic, v_HRMED_EarnDedFlagBasic, 'Amount', v_HRMED_RoundOffFlag);
        
        RAISE NOTICE 'BASICPAY Amount: %', v_BASICPAY;
        RAISE NOTICE 'Basic salary is inserted into temp';
        ...
        WHERE "MI_Id" = p_MI_ID AND "HRME_Id" = p_HRME_ID 
        AND "HRES_Year" = v_LEAVE_YEAR AND "HRES_Month" = p_IVRM_Month_Name;
        
        IF v_HRES_ID > 0 THEN
            IF v_HRC_SalApprovalFlg = TRUE THEN
                UPDATE "HR_Employee_Salary" 
                SET "MI_Id" = p_MI_ID, "HRME_Id" = p_HRME_ID, "HRES_Year" = v_LEAVE_YEAR, 
                    "HRES_Month" = p_IVRM_Month_Name, "HRES_WorkingDays" = v_WORKINGDAYS, 
                    "HRES_DailyRates" = v_SALARYPERDAY, "HRES_EPF" = NULL, "HRES_FPF" = NULL, 
                    "HRES_Ac21" = NULL, "HRES_Ac22" = NULL, "HRES_Ac5" = NULL,
                    "HRES_FromDate" = v_MONTH_START_DATE::DATE, "HRES_ToDate" = v_MONTH_END_DATE::DATE,
                    "HRMGT_Id" = v_EMPHRMGT_Id, "HRMD_Id" = v_EMPHRMD_Id, "HRMDES_Id" = v_EMPHRMDES_Id,
                    "HRES_BankCashFlag" = v_HRME_PaymentType, "HRES_BankCode" = v_HRMBD_BankName, 
        ...
        ELSE
            IF v_HRC_SalApprovalFlg = TRUE THEN
                INSERT INTO "HR_Employee_Salary"("MI_Id", "HRME_Id", "HRES_Year", "HRES_Month", 
                    "HRES_WorkingDays", "HRES_DailyRates", "HRES_EPF", "HRES_FPF", "HRES_Ac21", 
                    "HRES_Ac22", "HRES_Ac5", "HRES_FromDate", "HRES_ToDate", "HRES_ArrearRegFlag", 
                    "HRES_BankCashFlag", "HRMGT_Id", "HRMD_Id", "HRMDES_Id", "HRES_BankCode", 
                    "HRES_AccountNo", "HRES_ESIEmplr", "HRES_ApproveFlg", "HRES_CreatedDate", "HRES_UpdatedDate") 
                VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, 
                    v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, 
                    v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, 
                    v_EMPHRMDES_Id, v_HRMBD_BankName, v_HRMEB_AccountNo, NULL, FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            ...
            ELSE
                ...
                VALUES (p_MI_ID, p_HRME_ID, v_LEAVE_YEAR, p_IVRM_Month_Name, v_WORKINGDAYS, 
                    v_SALARYPERDAY, NULL, NULL, NULL, NULL, NULL, v_MONTH_START_DATE::DATE, 
                    v_MONTH_END_DATE::DATE, NULL, v_HRME_PaymentType, v_EMPHRMGT_Id, v_EMPHRMD_Id, 
                    v_EMPHRMDES_Id, v_HRMBD_BankName, v_HRMEB_AccountNo, NULL, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                RETURNING "HRES_Id" INTO v_NEW_IDENTITY;
            END IF;
        END IF;
        
        FOR rec IN
            SELECT A."HRMED_Id", A."HREED_