CREATE OR REPLACE FUNCTION "dbo"."FO_sp_mg_paycare_latein_details_Mail" (p_miid bigint)
RETURNS TABLE (
    "SL NO." bigint,
    "HRME_Id" bigint,
    ...
    FOR cur_record IN (
        SELECT * FROM (
            SELECT DISTINCT 
                ...
                c."FOEST_IHalfLoginTime" as "LoginTime",
                "dbo"."getdatediff"("dbo"."mintotime"(("dbo"."getonlymin"(c."FOEST_IHalfLoginTime"))),j."FOEPD_PunchTime") as "LATE BY",
                '00:00' as earlyby,
                "FOEST_IIHalfLogoutTime" AS "LogoutTime"
            ...
            FROM (
                ...
                FROM (
                    ...
                    "HRME_EmailId","HRME_MobileNo",b."HRMD_DepartmentName" as depname,c."HRMDES_DesignationName" as desgname,d."HRMGT_EmployeeGroupType" as gtype 
        ...
        IF v_emp_code1 <> v_emp_code THEN
            INSERT INTO "MG_DB_late_in_temp_New"("EMP_CODE","EMP_NAME","EMP_DEPARTMENT","EMP_DESIGNATION","EMP_GRADE","punchtime","shifttime","LATE_BY","REMARKS","EmailId","MobileNo","LogoutTime")
