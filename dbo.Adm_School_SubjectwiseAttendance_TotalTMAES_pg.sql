CREATE OR REPLACE FUNCTION "dbo"."Adm_School_SubjectwiseAttendance_TotalTMAES"(
    p_MI_Id bigint,
    p_ASMAY_Id bigint,
    p_ASMCL_Id bigint,
    p_ASMS_Id bigint,
...
RETURNS TABLE (
    ...
    "Absentcount" numeric,
    "StudentPercentage" numeric(18,2)
)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        CAST(("New"."ASA_Class_Attended"::numeric / "New"."ASA_ClassHeld"::numeric * 100) AS numeric(18,2)) AS "StudentPercentage"
    FROM (
        SELECT DISTINCT
            "ASAS"."ISMS_Id",
            ...
            COUNT(CASE WHEN "ASA_Class_Attended" = 1.00 THEN 1 END) AS "ASA_Class_Attended",
            SUM("ASA_ClassHeld") AS "ASA_ClassHeld"
        ...
        INNER JOIN "Adm_School_PeriodwiseAttendancedatesTotal_Temp" "PAT" ON "PAT"."ADates" = CAST("ASA_FromDate" AS DATE)
        ...
        AND CAST("ASA_FromDate" AS DATE) >= CAST(p_FromDate AS DATE)
        AND CAST("ASA_ToDate" AS DATE) <= CAST(p_ToDate AS DATE)
