CREATE OR REPLACE FUNCTION "dbo"."FA_BalanceSheetNew"(
    p_IMFY_Id bigint,
    p_MI_Id bigint,
    p_FAMCOMP_Id bigint,
    p_sdate varchar(10),
...
BEGIN
    ...
    DROP TABLE IF EXISTS "dbo"."tmpallgroupBS";
    ...
    DROP TABLE IF EXISTS "dbo"."tmpallgroupBSSkip";
    ...
    FOR rec_maingroup IN 
        ...
        WHERE "FAMGRP_BSPLFlg" = 'BS' 
          AND "FAMGRP_ParentId" = 0 
          AND "MI_Id" = p_MI_Id
          AND "FAMGRP_Id" IN (49, 42, 55)
        ORDER BY "FAMGRP_Id"
    LOOP
    ...
    FOR rec_ledgroup IN 
        SELECT DISTINCT "FAMLED_Id", "FAMLED_LedgerName", a."FAMGRP_Id", "FAMGRP_GroupName", 
               "grouplevel", b."FAMGRP_ParentId", "FAMGRP_CRDRFlg", CAST(b."FAMGRP_Position" AS bigint) AS "FAMGRP_Position"
    ...
    LOOP
        ...
        v_FAMGRP_Positionnew := rec_ledgroup."FAMGRP_Position";
        ...
        IF v_l_code IS NOT NULL THEN
            ...
            FROM "dbo"."FA_OpeningbalanceSingLeAcc"(v_l_code, p_MI_Id, p_FAMCOMP_Id, v_groupcode, p_sdate);
        ...
        IF (v_IntTranDr > v_IntTranCr) THEN
            INSERT INTO "tmpBS" VALUES (v_groupcode, v_groupname, v_levelcode, v_parentid, v_L_Name, 
                                        v_l_code, v_mg_type, v_IntTranDr - v_IntTranCr, v_FAMGRP_Positionnew);
        ELSE
            INSERT INTO "tmpBS" VALUES (v_groupcode, v_groupname, v_levelcode, v_parentid, v_L_Name, 
                                        v_l_code, v_mg_type, v_IntTranCr - v_IntTranDr, v_FAMGRP_Positionnew);
        END IF;
    END LOOP;
    -- Ledger Insert End
    ...
    RETURN QUERY
    SELECT a."FAMGRP_Id", a."FAMGRP_GroupName", a."levelcodenew", a."FAMGRP_ParentId", 
           a."FAMLED_LedgerName", a."FAMLED_Id", a."mg_type", a."Amt", a."FAMGRP_Position",
           (SELECT COUNT(*) FROM "Balancesheettemp" WHERE "FAMGRP_ParentId" = a."FAMGRP_Id") AS "ChildRecords"
    FROM "Balancesheettemp" a 
    ORDER BY a."FAMGRP_Position";
