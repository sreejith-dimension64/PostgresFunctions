CREATE OR REPLACE FUNCTION "dbo"."FO_Emp_Monthly_yearly_Report_New"(
    "fromdate" VARCHAR(10),
    "todate" VARCHAR(10),
    "multiplehrmeid" TEXT,
    "miid" BIGINT,
...
BEGIN
    ...
    IF "type" = 'monthly' THEN
        ...
        "var1" := '';
        SELECT STRING_AGG(DISTINCT '"' || dt || '"', ',' ORDER BY '"' || dt || '"')
        INTO "cols"
    ...
    ELSE
        ...
        SELECT STRING_AGG(DISTINCT '"' || dt || '"', ',' ORDER BY dt1)
        INTO "cols"
        FROM (
    ...
    IF ("M" = 'Y') OR ("M" = 'N') THEN
        ...
        WHERE A."col"::DATE = B."FOMHWDD_FromDate";

        "dynamic" := 'SELECT "ecode", "ename", "HRME_DOJ", "MonthEmpList"."HRME_Id", "tpdays", 
                      "HRMDES_DesignationName", "hwkdays", ' || "cols" || ', 
                      SUM("dbo"."getonlymin"(EP."FOEP_LateByMins")) AS "LateByMins"
                      FROM (
                          SELECT * FROM CROSSTAB(
                              ''SELECT "HRME_Id", "col", SUM("value")
                                FROM "Empfomonthly_New"
                                GROUP BY "HRME_Id", "col"
                                ORDER BY 1, 2'',
                              ''SELECT UNNEST(ARRAY[' || "cols" || ']::TEXT[])''
                          ) AS pvt("HRME_Id" BIGINT, "ecode" VARCHAR(100), "ename" TEXT, 
                                   "HRME_DOJ" VARCHAR(10), "tpdays" INT, 
                                   "HRMDES_DesignationName" VARCHAR(100), "hwkdays" INT, ' || "cols" || ' INT)
                      ) "MonthEmpList"
                      ...
                      GROUP BY "MonthEmpList"."ecode", "MonthEmpList"."ename", "MonthEmpList"."HRME_DOJ", 
                               "MonthEmpList"."HRME_Id", "MonthEmpList"."tpdays", 
                               "MonthEmpList"."HRMDES_DesignationName", "MonthEmpList"."hwkdays", ' || "cols";
    ...
    ELSIF ("M" = 'A') THEN
        ...
        "query" := 'SELECT DISTINCT "HRME_Id", "ecode", "ename", "HRME_DOJ", "HRMDES_DesignationName", 
                    "workday", "tpdays", ("workday" - "tpdays") AS "absentdays"
                    FROM (
                        SELECT DISTINCT ES."HRME_Id", ES."HRME_EmployeeCode" AS "ecode",
                               (COALESCE(ES."HRME_EmployeeFirstName", '' '') || '' '' || 
                                COALESCE(ES."HRME_EmployeeMiddleName", '' '') || '' '' || 
                                COALESCE(ES."HRME_EmployeeLastName", '' '')) AS "ename",
                               ES."HRME_DOJ",
                        ...
                        AND ES."HRME_Id" NOT IN (
                            ...
                            FROM (
                                SELECT DISTINCT e."HRME_EmployeeCode" AS "ecode",
                                       (COALESCE(e."HRME_EmployeeFirstName", '' '') || '' '' || 
                                        COALESCE(e."HRME_EmployeeMiddleName", '' '') || '' '' || 
                                        COALESCE(e."HRME_EmployeeLastName", '' '')) AS "ename",
                                       e."HRME_DOJ",
                                       D."HRMDES_DesignationName",
                                       a."FOEP_Id",
                                ...
                                GROUP BY e."HRME_EmployeeCode", e."HRME_EmployeeFirstName", e."HRME_EmployeeMiddleName",
                                         e."HRME_EmployeeLastName", e."HRME_DOJ", D."HRMDES_DesignationName",
                                         a."FOEP_Id", CAST(a."FOEP_PunchDate" AS DATE), a."HRME_Id"
                            ) AS "HRME"
                        )
                        OR ES."HRME_Id" IN (
                            SELECT DISTINCT "HRME_Id"
