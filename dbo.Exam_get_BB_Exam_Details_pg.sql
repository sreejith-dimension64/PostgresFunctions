CREATE OR REPLACE FUNCTION "dbo"."Exam_get_BB_Exam_Details"(
    p_ASMAY_Id bigint,
    p_ASMCL_Id bigint,
    p_ASMS_Id bigint,
    p_MI_Id bigint,
    ...
    IF (v_ExmConfig_LeftStuFlag = false) THEN
        ...
        SELECT DISTINCT 
            ...
            (SELECT sum("asa_classheld") FROM "Adm_Student_Attendance" p 
             ...
             AND "ASMCL_Id" = p_ASMCL_Id AND "ASA_Activeflag" = true AND "ASMS_Id" = p_ASMS_Id 
             AND ((p."ASA_FromDate" BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate")
             OR (p."ASA_ToDate" BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate"))) AS "ASA_ClassHeld",
            (SELECT sum("ASA_Class_Attended") FROM "Adm_Student_Attendance_Students" q, "Adm_Student_Attendance" as p
             ...
             AND "ASA_Activeflag" = true AND "ASMCL_Id" = p_ASMCL_Id AND "ASMS_Id" = p_ASMS_Id 
             AND q."AMST_Id" = f."AMST_Id" 
             AND ((p."ASA_FromDate" BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate")
             OR (p."ASA_ToDate" BETWEEN l."EYCE_AttendanceFromDate" AND l."EYCE_AttendanceToDate"))) AS "ASA_Class_Attended",
            r."EMGD_Remarks",
            ...
            (SELECT COALESCE(SUM(Eyck."EYCES_MinMarks"), 0) 
             ...
             AND exc."ASMAY_Id" = p_ASMAY_Id AND exc."EYC_ActiveFlg" = true
             INNER JOIN "Exm"."Exm_Yearly_Category_Exams" as eyc ON eyc."EYC_Id" = exc."EYC_Id" 
             AND eyc."EME_Id" = p_EME_Id AND eyc."EYCE_ActiveFlg" = true
             INNER JOIN "Exm"."Exm_Yrly_Cat_Exams_Subwise" as Eyck ON eyc."EYCE_Id" = Eyck."EYCE_Id" 
             AND Eyck."EYCES_AplResultFlg" = true AND eyck."EYCES_ActiveFlg" = true
             ...
             AND "EME_Id" = p_EME_Id AND "ASMS_Id" = p_ASMS_Id AND "ASMCL_Id" = p_ASMCL_Id)) as "stu_grandmin_marks"
        ...
        AND j."MI_Id" = p_MI_Id AND j."ECAC_ActiveFlag" = true
        INNER JOIN "Exm"."Exm_Yearly_Category" as k ON j."EMCA_Id" = k."EMCA_Id" 
        AND k."ASMAY_Id" = p_ASMAY_Id AND k."MI_Id" = p_MI_Id AND k."EYC_ActiveFlg" = true
        INNER JOIN "Exm"."Exm_Yearly_Category_Exams" as l ON l."EYC_Id" = k."EYC_Id" 
        AND l."EME_Id" = p_EME_Id AND l."EYCE_ActiveFlg" = true
        ...
        LEFT OUTER JOIN "Exm"."Exm_Student_Marks_Process_Subjectwise" as a 
        ON m."ISMS_Id" = a."ISMS_Id" AND f."AMST_Id" = a."AMST_Id" 
        ...
        LEFT OUTER JOIN "Exm"."Exm_Master_Grade_Details" as r 
        ON m."emgr_id" = r."emgr_id" AND a."ESTMPS_ObtainedGrade" = r."EMGD_Name"
        LEFT OUTER JOIN "Exm"."Exm_Master_Grade_Details" as t 
        ON l."emgr_id" = t."emgr_id" AND s."ESTMP_TotalGrade" = t."EMGD_Name"
        INNER JOIN "Exm"."Exm_Master_Exam" as c ON l."EME_Id" = c."EME_Id" 
        AND c."MI_Id" = p_MI_Id AND c."EME_ActiveFlag" = true
        ...
        ORDER BY f."AMST_FirstName", f."AMST_MiddleName", f."AMST_LastName", 
        m."EYCES_SubjectOrder" ASC;
    ELSE
        IF (v_ExmConfig_LeftStuFlag = true) THEN
            ...
            AND n."ASMAY_Id" = p_ASMAY_Id AND n."ASMCL_Id" = p_ASMCL_Id 
            AND n."ASMS_Id" = p_ASMS_Id
            INNER JOIN "EXM"."Exm_Student_Marks" as p ON p."AMST_Id" = h."AMST_Id" 
            AND p."ASMAY_Id" = n."ASMAY_Id" AND h."ASMS_Id" = p."ASMS_Id" 
            AND p."ASMCL_Id" = h."ASMCL_Id" AND p."ISMS_Id" = d."ISMS_Id" 
            AND p."ASMAY_Id" = p_ASMAY_Id AND p."MI_Id" = p_MI_Id
            ORDER BY f."AMST_FirstName", f."AMST_MiddleName", f."AMST_LastName", 
            m."EYCES_SubjectOrder" ASC;
        END IF;
    END IF;
