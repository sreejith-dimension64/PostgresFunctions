CREATE OR REPLACE FUNCTION "dbo"."DATEWISE_TRN_CD_FEE"(
   "@MI_Id" bigint,
   "@ASMAY_Id" bigint,
   "@frmdate" date,
   "@todate" date
...
BEGIN
   RETURN QUERY
   SELECT DISTINCT C."FMT_Id", B."TRMR_Id", 
   ...
   WHERE A."FMH_Id" IN (
      ...
      WHERE "FMH_Id" IN (
         ...
         WHERE "MI_Id" = "@MI_Id" AND "FMH_Flag" = 'T' AND "FMH_RefundFlag" = 1
      )
   ) 
   AND A."MI_Id" = "@MI_Id" 
   ...
   AND A."AMST_Id" IN (
      ...
      INNER JOIN "dbo"."Fee_Y_Payment" G ON G."MI_Id" = "@MI_Id" 
         AND G."ASMAY_Id" = "@ASMAY_Id" 
         AND G."fyp_id" = F."fyp_id"
      INNER JOIN "Fee_T_Payment" AS H ON H."FYP_Id" = G."FYP_Id"
      INNER JOIN "Fee_Master_Amount" AS E ON E."MI_Id" = "@MI_Id" 
         AND E."ASMAY_Id" = "@ASMAY_Id" 
         AND H."FMA_Id" = E."FMA_Id" 
      WHERE CAST(G."FYP_Date" AS date) BETWEEN "@frmdate" AND "@todate"
   )
