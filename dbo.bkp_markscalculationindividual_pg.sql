CREATE OR REPLACE FUNCTION "Exm"."bkp_markscalculationindividual"(
    "p_MI_Id" bigint,
    "p_ASMAY_Id" bigint,
    "p_ASMCL_Id" bigint,
    "p_ASMS_Id" bigint,
...
DECLARE
    ...
    "v_ClassRankFlg" boolean;
    "v_SectionRankFlg" boolean;
    "v_FailRankFlg" boolean;
    "v_ClassPositionFlg" boolean;
    "v_SectionPositionFlg" boolean;
    "v_GroupMaxMarks" decimal(18,2);
    "v_GroupObtainedMarks" decimal(18,2);
    "v_FailcountG" int;
    "v_AbsentcountG" int;
    "v_MedicalcountG" int;
    "v_SportscountG" int;
    "v_GroupwiseMarksFlg" boolean;
    "v_PSMaxMarks" decimal(18,2);
    "v_ESG_Id" bigint;
    "v_ESTMPS_GradePoints" decimal(18,2);
    "v_ESTMP_GradePoints" decimal(18,2);
    "v_MI_Id" bigint;
    "v_ASMAY_Id" bigint;
    "v_ASMCL_Id" bigint;
    "v_ASMS_Id" bigint;
    ...
    "v_ODcountU" bigint;
    "v_GroupMaxMarks_Max" decimal(18,2);
    "v_GroupObtainedMarks_New" decimal(18,2);
    "rec_exmsubjectdetails" record;
    "rec_ExamSubjectwisecalculation" record;
    "rec_StudentMarksProcess" record;
    "rec_groupids" record;
    "rec_AllSubAbsentStudentsUp" record;
BEGIN

    DELETE FROM "Exm"."Exm_Student_Marks_Pro_Sub_SubSubject" 
    WHERE "ESTMPS_Id" IN(
        SELECT "ESMPS"."ESTMPS_Id" 
    ...
    DELETE FROM "Exm"."Exm_StudentWiseSubjectGroupMarks" 
    WHERE "MI_Id"="p_MI_Id" AND "ASMAY_Id"="p_ASMAY_Id" 
    AND "ASMCl_Id"="p_ASMCL_Id" AND "ASMS_Id"="p_ASMS_Id" 
    AND "EME_Id"="p_EME_Id";

    SELECT "ExmConfig_RankingMethod", "ExmConfig_ClassRankFlg", 
           "ExmConfig_SecRankFlg", "ExmConfig_FailRankFlg",
           "ExmConfig_ClassPositionFlg", "ExmConfig_SectionPositionFlg"
    INTO "v_ExmConfig_RankingMethod", "v_ClassRankFlg", "v_SectionRankFlg",
         "v_FailRankFlg", "v_ClassPositionFlg", "v_SectionPositionFlg"
    ...
    DROP TABLE IF EXISTS "temp_Exmsubjectdetails";

    CREATE TEMP TABLE "temp_Exmsubjectdetails" AS
    SELECT DISTINCT "ESM"."AMST_Id", "ESM"."MI_Id", "ESM"."ASMAY_Id", 
           "ESM"."ASMCL_Id", "ESM"."ASMS_Id", "ESM"."ISMS_Id", "ESM"."EME_Id",
           "EYCES_AplResultFlg", "EYCES_MarksEntryMax", "EYCES_MaxMarks",
           "EYCES_MinMarks", "EYCES"."EMGR_Id", "ESM"."ESTM_Marks",
    ...
    INNER JOIN "Adm_School_Y_Student" AS "h" ON "h"."AMST_Id" = "f"."AMST_Id" 
        AND "f"."AMST_ActiveFlag" = true AND "f"."AMST_SOL" = 'S' 
        AND "h"."AMAY_ActiveFlag" = true AND "h"."ASMAY_Id" = "p_ASMAY_Id" 
        AND "h"."ASMCL_Id" = "p_ASMCL_Id" AND "h"."ASMS_Id" = "p_ASMS_Id" 
        AND "f"."mi_id"="p_MI_Id"
    ...
    INNER JOIN "Exm"."Exm_Studentwise_Subjects" AS "n" ON "n"."ISMS_Id" = "EYCES"."ISMS_Id" 
        AND "n"."AMST_Id"="f"."AMST_Id" AND "n"."MI_Id" = "p_MI_Id" 
        AND "n"."ASMAY_Id" = "p_ASMAY_Id" AND "n"."ASMCL_Id" = "p_ASMCL_Id" 
    ...
    FOR "rec_exmsubjectdetails" IN 
        SELECT * FROM "temp_Exmsubjectdetails"
    LOOP
        ...
        IF ("v_ESTM_Flg"='AB') THEN
            SELECT COUNT("ESTMSS_Marks")
            INTO "v_ESTMMSMarksCount"
        ...
        IF "v_ESTM_MarksGradeFlg"='M' THEN
            ...
            SELECT "EMGD_Name", "EMGD_GradePoints" 
            INTO "v_ESTMPS_ObtainedGrade", "v_ESTMPS_GradePoints"
            FROM "Exm"."Exm_Master_Grade_Details" 
            WHERE ((CAST("v_GradeMarksPercentage" AS decimal) BETWEEN "EMGD_From" AND "EMGD_To")) 
                AND "EMGR_Id"="v_EMGR_Id"
            LIMIT 1;
        ELSIF "v_ESTM_MarksGradeFlg"='G' THEN
        ...
        INSERT INTO "Exm"."Exm_Student_Marks_Process_Subjectwise" 
            ("MI_Id", "ASMAY_Id", "ASMCL_Id", "ASMS_Id", "AMST_Id", "ISMS_Id", "EME_Id",
             ...
             "ESTMPS_AplResultFlg", "ESTMPS_GradePoints")
        VALUES("p_MI_Id", "p_ASMAY_Id", "p_ASMCL_Id", "p_ASMS_Id", "v_AMST_Id", "v_ISMS_Id", 
               "p_EME_Id", "v_EYCES_MaxMarks", "v_ESTM_Marks", "v_ESTMPS_ObtainedGrade",
               "v_ESTMPS_PassFailFlg", CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, "v_EYCES_MaxMarks",
               "v_EYCES_AplResultFlg", "v_ESTMPS_GradePoints");
        ...
        IF ("v_EYCES_SubExamFlg"=false AND "v_EYCES_SubSubjectFlg"=true) 
            ...
            PERFORM "Exm"."SubSubject_SubExam_New"("p_MI_Id", "p_ASMAY_Id", "p_ASMCL_Id", 
                                                    "p_ASMS_Id", "v_AMST_Id", "p_EME_Id", "v_ISMS_Id");
        END IF;

    END LOOP;

    FOR "rec_ExamSubjectwisecalculation" IN
    ...
    LOOP
        "v_MI_Id" := "rec_ExamSubjectwisecalculation"."MI_Id";
        "v_ASMAY_Id" := "rec_ExamSubjectwisecalculation"."ASMAY_Id";
        "v_ASMCL_Id" := "rec_ExamSubjectwisecalculation"."ASMCL_Id";
        "v_ASMS_Id" := "rec_ExamSubjectwisecalculation"."ASMS_Id";
        "v_ISMS_Id" := "rec_ExamSubjectwisecalculation"."ISMS_Id";
        "v_EME_Id" := "rec_ExamSubjectwisecalculation"."EME_Id";

        SELECT SUM("ESTMPS_ObtainedMarks") 
        INTO "v_Section_Totalmarks"
        FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"="v_MI_Id" AND "ASMAY_Id"="v_ASMAY_Id" 
            AND "ASMCL_Id"="v_ASMCL_Id" AND "ASMS_Id"="v_ASMS_Id" 
            AND "EME_Id"="v_EME_Id" AND "ISMS_Id"="v_ISMS_Id"
        GROUP BY "ASMCL_Id", "ASMS_Id", "ISMS_Id";

        SELECT COUNT("AMST_Id") 
        INTO "v_Section_Totalcount"
        FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"="v_MI_Id" AND "ASMAY_Id"="v_ASMAY_Id" 
            AND "ISMS_Id"="v_ISMS_Id" AND "ASMCL_Id"="v_ASMCL_Id" 
            AND "ASMS_Id"="v_ASMS_Id" AND "EME_Id"="v_EME_Id";
        ...
        SELECT SUM("ESTMPS_ObtainedMarks") 
        INTO "v_Class_Totalmarks"
        FROM "Exm"."Exm_Student_Marks_Process_Subjectwise" 
        WHERE "MI_Id"="v_MI_Id" AND "ASMAY_Id"="v_ASMAY_Id" 
            AND "ASMCL_Id"="v_ASMCL_Id" AND "EME_Id"="v_EME_Id" 
            AND "ISMS_Id"="v_ISMS